...
def schedule_reminders_for_play(play):
    ...
    total   = play["total"]
    tier    = play["tier"]
    game_dt = play["time_str"]

    key     = make_play_key(league, p1, p2, play["time_str"])
    now_est = datetime.now(EST)

    # Already scheduled — skip
    guild_id = play.get('guild_id') if isinstance(play, dict) else None
    ...
    for raw_line in text.split("\n"):
        ...
        if play:
            keys.add(make_play_key(
                play["league"], play["p1"], play["p2"], play["time_str"]
            ))
    return keys
...
async def schedule_reminders_from_text(text):
    ...
    for line, time_str in raw_plays:
        ...
        if not play:
            continue
        play["time_str"] = game_dt  # override with batch-corrected datetime
...
# ==============================
    ...
    if content=="!reminders":
        ...
        lines = [f"⏰ **ACTIVE REMINDERS** ({len(scheduled_tasks[guild_id])} play(s)) ━━━━━━━━━━━━━━━━━━"]
        ...
        for key in scheduled_tasks[guild_id]:
            try:
                ...
                lines.append(
                    f"{league} – {p1} vs {p2} @ {time_str} EST\n→ in {h}h {m}m"
                )
