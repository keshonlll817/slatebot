...
async def on_message(message):
    ...
    if content.startswith("!disable"):
        ...
        if "true" in content:
            BOT_DISABLED = True
            await message.channel.send("🔒 Bot disabled in main server.")
        elif "false" in content:
            BOT_DISABLED = False
            await message.channel.send("✅ Bot enabled in all servers.")
        return
...
# ==============================
    ...
    if content=="!reminders":
        ...
        for key in sorted(scheduled_tasks.keys()):
            ...
            lines.append(f"{league_k} – {p1_k} vs {p2_k} @ {time_k} EST")
