    ...
    while not client.is_closed():
        try:
            ...
            for guild_id, alerts in list(pending_alerts.items()):
                ...
                # Find alerts that are due (within a 90s window so nothing is missed)
                due = [a for a in alerts
                       if not a["sent"] and a["fire_at"] <= now and (now - a["fire_at"]).total_seconds() <= 90]
                ...
                for (ch_id, label), group in groups.items():
                    ...
                    for a in group:
                        ...
                        lines.append(build_reminder_text(
                            ...
                            p["wins"], p["total"], p["tier"], label, p.get("play_type", "")
                        ))

                    if len(lines) > 1:
                        # One ping at top, strip individual role mentions
                        role_mention = ""
                        role_m = re.match(r'(<@&\d+>)', lines[0])
                        if role_m:
...
async def on_ready():
    ...
    # Start the central reminder dispatcher
