...
def _destroy_to_dt(parsed, anchor_dt):
    ...
    if ampm == "PM" and hh != 12:
        ...
        # No meridiem given (e.g. "legs: 2:35 EST"). TT runs around the
        # clock, so pick whichever reading is nearest the post time.
        best, best_gap = None, None
        for cand_h in (hh, hh + 12 if hh < 12 else hh - 12):
            c = anchor_dt.replace(hour=cand_h % 24, minute=mm, second=0, microsecond=0)
            if c < anchor_dt - timedelta(hours=1):
                c += timedelta(days=1)
            gap = abs((c - anchor_dt).total_seconds())
...
_LIXX_STAKE_RE = re.compile(r'(?<![\d.])(\d+(?:\.\d+)?)\s*U\b', re.IGNORECASE)
