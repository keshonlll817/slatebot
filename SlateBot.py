...
# ╔══════════════════════════════════════════════════════════════╗
# ║  ONE-TIME DEMO — REMOVE BEFORE THE NEXT BUILD                ║
# ║                                                              ║
# ║  Fires the review nudge once, ~90s after this build boots,   ║
# ║  ignoring the hour window and the 14-day wait, then holds    ║
# ║  the daily-units nudge for 15 minutes so both can be seen    ║
# ║  back to back instead of waiting two weeks.                  ║
# ║                                                              ║
# ║  Afterwards the normal cycle resumes untouched: the next     ║
# ║  review nudge is 14 days out and lands in the OTHER channel. ║
# ║  It self-disables after firing (persisted), so a restart     ║
# ║  cannot double-send.                                         ║
# ║                                                              ║
# ║  >>> DELETE FIRST_RUN_DEMO, DEMO_UNITS_DELAY_MINUTES, the    ║
# ║      _first_run_demo() function, and its two call sites.     ║
# ╚══════════════════════════════════════════════════════════════╝
FIRST_RUN_DEMO           = True
DEMO_UNITS_DELAY_MINUTES = 15
...
engage_state = {
    ...
    "demo_done": False,          # ONE-TIME DEMO — remove with FIRST_RUN_DEMO
    "demo_units_after": None,    # ONE-TIME DEMO — remove with FIRST_RUN_DEMO
...
async def _engagement_loop():
    ...
    # ONE-TIME DEMO — remove this block with FIRST_RUN_DEMO
    if FIRST_RUN_DEMO and not engage_state.get("demo_done"):
        try:
            await _first_run_demo()
        except Exception:
            log.exception("[ENGAGE] DEMO first-run sequence failed:")
...
async def _first_run_demo():
    """ONE-TIME: fire the review nudge now, hold units 15 min. REMOVE ME."""
    now = datetime.now(EST)
    engage_state["demo_done"] = True
    engage_state["demo_units_after"] = (
        now + timedelta(minutes=DEMO_UNITS_DELAY_MINUTES)).isoformat()
    # Force the nudge past the hour window and the interval gate.
    engage_state["review_target_hour"] = now.hour
    engage_state["review_last_sent"] = None
    _engage_save()
    await _check_review()
    log.warning(f"[ENGAGE] DEMO review nudge fired on first boot; daily-units "
                f"nudge held until {(now + timedelta(minutes=DEMO_UNITS_DELAY_MINUTES)):%I:%M %p}. "
                f"Normal 14-day cycle resumes — next review goes to channel "
                f"{engage_state.get('review_next_channel')}.")
...
async def _check_daily_units():
    # ONE-TIME DEMO — remove this block with FIRST_RUN_DEMO
    after = engage_state.get("demo_units_after")
    if after:
        try:
            if datetime.now(EST) < datetime.fromisoformat(after):
                return
            engage_state["demo_units_after"] = None
            _engage_save()
        except Exception:
            engage_state["demo_units_after"] = None
...
async def _post_auto_recap(kind, anchor):
    ...
    recap += _format_ungraded_footer(
        await scan_ungraded_four_plus(four_ch, start, end, None), "play")
...
async def _all_graded_for(start, end):
    """True when nothing in the window is still awaiting a grade."""
    ch = await _fetch_ch_safe(FOUR_PLUS_CHANNEL)
    if ch is None:
        return False
    return not await scan_ungraded_four_plus(ch, start, end, None)
