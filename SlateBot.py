...
def parse_recap_text(text):
    ...
    if "\U0001f4ca **" not in text or "RECAP" not in text:
        return None
    data = {"title": None, "four_plus": None, "totals": None, "leagues": []}
    title_m = re.search(r'\U0001f4ca \*\*(.+?)\*\*', text)
    if title_m:
        data["title"] = title_m.group(1)
    sections = re.split(r'\U0001f3d3 \*\*(.+?)\*\*', text)
...
async def on_message(message):
    ...
    if (not message.author.bot and
            message.channel.id in (FOUR_PLUS_CHANNEL, TOTALS_CHANNEL) and
            "📊 **" in message.content and "RECAP" in message.content):
        ...
        if recap_data:
            try:
                ...
                print(f"[RECAP] Converted plain text recap to embed in channel {message.channel.id}")
            except Exception as e:
...
@discord.app_commands.describe(period="today, yesterday, weekly, lastweek, monthly, lifetime, ytd, best, worst, test, or a date like Jan 12 2026")
async def slash_recap(interaction: discord.Interaction, period: str = "today"):
    ...
    if p == "test":
        ...
        for prd in ["day", "week", "month", "year"]:
            ...
            if entry:
                ...
                tu = d["tunits"]
                net = fu + tu
                out += f"{icon} **{mode} {prd.upper()}:** {label}\n"
                out += f"4+: {d['fw']}-{d['fl']} | {fu:+.2f}U   Totals: {d['tw']}-{d['tl']} | {tu:+.2f}U\n"
                out += f"**Net: {net:+.2f}U**\n\n"
            else:
                out += f"{icon} **{mode} {prd.upper()}:** No data\n\n"
        ...
        if s is None:
            await interaction.response.send_message("❌ Unknown recap period. Try: today, yesterday, weekly, monthly, lifetime, ytd, best, worst, or a date like Jan 12 2026", ephemeral=True)
            return
