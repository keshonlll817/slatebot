...
async def mirror_four_recap(recap_data):
    ...
    try:
        ...
        if ch is None:
            log.error(f"[MIRROR] Channel {FREE_RECAP_MIRROR_CHANNEL} unreachable")
            return False
        four_only = {
        ...
        log.info(f"[MIRROR] 4+ recap copied to free channel: {recap_data['title']}")
        return True
    except Exception:
...
def _norm_line(line):
    return re.sub(r'\s+', ' ', line).replace("\ufe0f", "").strip()
