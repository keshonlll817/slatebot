...
LIXX_CHANNEL_IDS = {
    1506366131953209574,    # general-locks
    1511010434487156899,    # mlb-locks
    1511011187498680494,    # soccer-locks
    1511011330730102804,    # nfl-locks
}
...
async def _run_lixx_recap(channel, arg):
    """Shared engine for !lixx and /lixx. Returns (text, ok)."""
    l_start, l_end, l_title = _parse_lixx_period(arg)
    if l_start is None and l_title is None:
        return ("Usage: `/lixx [period]` — or `!lixx [today|yesterday|weekly|lastweek|monthly|ytd|lifetime|<date>]`", False)

    l_stats = await parse_generic_channel(channel, l_start, l_end)

    out = f"📊 **{_lixx_title_base(channel.name)} RECAP — {l_title}**\n\n"
    l_total = l_stats["w"] + l_stats["l"] + l_stats["wash"]
    if l_total == 0:
        out += "No plays graded."
    else:
        out += f"Record: {l_stats['w']}-{l_stats['l']}"
        if l_stats["wash"] > 0:
            out += f" ({l_stats['wash']} Wash)"
        out += f"\nUnits: {l_stats['units']:+.2f}U"
        if l_stats["risked"] > 0:
            roi = l_stats["units"] / l_stats["risked"] * 100
            out += f"\nRisked: {l_stats['risked']:.1f}U  ({roi:+.1f}% ROI)"
    return (out, True)
...
def _schedule_play_for_message(guild_id, message_id, guild, play, game_dt, reminder_channel_id):
    ...
    log.info(f"[SCHEDULE] Scheduled: {key} @ {game_dt.strftime('%m/%d %I:%M %p')} EST (SOON {soon_dt.strftime('%I:%M %p')})")
    return key, True
...
# ==============================
    ...
    if content.startswith("!lixx"):
        ...
        out, _ok = await _run_lixx_recap(message.channel, arg)
...
@tree.command(name="lixx", description="LixX sports recap for this channel")
@discord.app_commands.describe(
    period="Select a recap period",
    date="Specific date (Jan 12 2026), month (April 2026), or year (2025)"
)
...
async def slash_lixx(interaction: discord.Interaction, period: discord.app_commands.Choice[str] = None, date: str = None):
    # Deliberately NOT gated by _check_rw — LixX and his subscribers don't
    # need the RW role; the channel gate below is the restriction.
    if not _is_lixx_channel(interaction.channel):
        await interaction.response.send_message("`/lixx` only works in LixX's channels.", ephemeral=True)
        return
    arg = (date or (period.value if period else "")).strip()
    await interaction.response.defer()
    text, _ok = await _run_lixx_recap(interaction.channel, arg)
