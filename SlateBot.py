...
async def on_ready():
    ...
    # Sync slash commands to specific guilds only (no global sync to avoid duplicates)
    # First clear any stale global commands
    try:
        tree.clear_commands(guild=None)
        await tree.sync()
        print("[SLASH] Cleared global commands.")
    except Exception as e:
        print(f"[SLASH] Could not clear global commands: {e}")
...
async def slash_freeplays(interaction: discord.Interaction):
    ...
    # Mention the Free Plays role and link to free-chat
    fp_role = None
    if interaction.guild:
        fp_role = discord.utils.get(interaction.guild.roles, id=FREE_PLAYS_ROLE_ID)
    if fp_role:
