...
async def slash_freeplays(interaction: discord.Interaction):
    ...
    # Scan today's messages only — stop at messages older than 36hrs
    plays = []
    now_est = datetime.now(EST)
    ...
    async for msg in four_ch.history(limit=200):
        ...
        for raw_line in msg.content.split("\n"):
            ...
            # Skip graded, non-play, header lines
            if "✅" in line or "❌" in line or "🧼" in line:
                continue
            if "vs" not in line:
                continue
            ...
            # Parse game time and skip if already passed
            ...
            if time_m:
                try:
                    ...
                    game_dt = naive.replace(tzinfo=EST)
                    # If game time is in the past, roll to tomorrow
                    if game_dt < now_est - timedelta(hours=1):
                        game_dt += timedelta(days=1)
                    # Skip if game already started (within 5 min buffer)
                    if game_dt <= now_est + timedelta(minutes=5):
                        continue
                except Exception:
                    pass  # Can't parse time, include the play anyway

            wins = int(record_m.group(1))
            total = int(record_m.group(2))
            pct = wins / total if total > 0 else 0
            plays.append({"line": line, "pct": pct, "league": league})

    if not plays:
        await interaction.followup.send("No upcoming ungraded plays found in today's 4+ slate.", ephemeral=True)
        return

    # Pick 2 random plays to keep it fair
    picks = random.sample(plays, min(2, len(plays)))

    text = ""
    for p in picks:
        text += p["line"] + "\n\n"

    # Role ping — use guild.get_role() for reliable cache lookup
    fp_role = None
    if interaction.guild:
        fp_role = interaction.guild.get_role(FREE_PLAYS_ROLE_ID)
        if fp_role is None:
            # Fallback: fetch guild fresh to ensure role cache is populated
            try:
                guild_fresh = await client.fetch_guild(interaction.guild_id)
                fp_role = guild_fresh.get_role(FREE_PLAYS_ROLE_ID)
            except Exception:
                pass

    if fp_role:
        text += f"{fp_role.mention} free plays! 4+ set method. Ask questions in <#{FREE_CHAT_CHANNEL_ID}> if you need a hand"
        mentions = discord.AllowedMentions(roles=[fp_role])
    else:
        text += f"free plays! 4+ set method. Ask questions in <#{FREE_CHAT_CHANNEL_ID}> if you need a hand"
        mentions = discord.AllowedMentions.none()
        print(f"[FREEPLAYS] Warning: Free Plays role {FREE_PLAYS_ROLE_ID} not found in guild")
    ...
    await interaction.followup.send(f"✅ Posted {len(picks)} free play(s) to <#{target_ch}>.", ephemeral=True)
