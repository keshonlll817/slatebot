...
REM_TICK_SECONDS   = 10
REM_SWEEP_SECONDS  = 600       # full re-sync from channel history
REM_SWEEP_LOOKBACK = 18        # hours of history the sweep re-reads
...
REMINDER_STATE_PATH = os.environ.get("REMINDER_STATE_PATH", "/tmp/slatebot_reminders.json")
...
def sync_reminders(guild_id, message_id, source, desired, dest_channel_id):
    ...
    for key, (game_dt, payload) in want.items():
        ...
        if existing["message_id"] != message_id:
            # Same fixture already tracked from another message — leave the
            # original owner alone rather than fighting over it.
            kept += 1
            log.debug(f"[REM] =DUP  {key} owned by msg {existing['message_id']}")
            continue
...
async def _reminder_dispatcher_loop():
    ...
    log.info(f"[REM] START dispatcher up | tick={REM_TICK_SECONDS}s "
             f"soon={REM_SOON_MINUTES}m grace={REM_LATE_GRACE}s state={REMINDER_STATE_PATH}")
