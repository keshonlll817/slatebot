...
async def _post_auto_recap(kind, anchor):
    ...
    recap += _format_ungraded_footer(
        await ungraded_started_plays(start, end), "play")

    await send_long_message(out_ch, recap)
    log.warning(f"[AUTORECAP] POSTED {kind} — {title}")

    data = parse_recap_text(recap)
    if data:
        await mirror_four_recap(data)
    else:
        log.warning(f"[AUTORECAP] {kind}: could not parse own output for mirroring")
    return True
