...
def parse_lixx_recap_text(text):
    ...
    sports = re.findall(r'^([A-Za-z][A-Za-z ]{1,18}):\s*(\d+)-(\d+)\s*\(([+\-]?\d+(?:\.\d+)?)U\)',
                        text, re.MULTILINE)
    ungraded = []
...
def build_lixx_recap_embed(d):
    ...
    for name, w, l, u in d["sports"]:
        ...
        embed.add_field(name=name, value=f"{s_icon} `{w}-{l}`  `{u:+.2f}U`", inline=True)
    return embed, color
...
async def _run_lixx_recap(sport_key, arg):
    ...
    out = f"{LIXX_RECAP_HEADER} — {scope} RECAP — {l_title}**\n\n"
...
async def on_message(message):
    ...
    if (not message.author.bot and _is_lixx_channel(message.channel)
