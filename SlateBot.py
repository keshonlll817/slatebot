...
# STARTING NOW is deleted this long after posting — roughly when a TT match
# has finished, so the channel doesn't keep alerts for games long over.
# STARTING SOON is intentionally never deleted.
...
async def _process_pending_deletes(now):
    """Delete STARTING NOW messages whose retention window has elapsed."""
    ...
    for d in ready:
        ...
        try:
            ...
            await msg.delete()
            log.info(f"[REM] DELETE removed NOW msg {d['message_id']} "
        ...
        try:
            ...
            for (gid, ch_id, label), items in groups.items():
                ...
                try:
                    ...
                    if label == "STARTING NOW" and getattr(sent_msg, "id", None):
                        ...
                        log.info(f"[REM] QUEUE delete of NOW msg {sent_msg.id} in "
...
def rem_diagnostics(guild_id):
    ...
    lines = ["🔎 **REMINDER ENGINE DIAGNOSTICS** ━━━━━━━━━━━━━━━━━━",
             ...
             f"(NOW alerts removed after `{REM_NOW_DELETE_MINUTES}m`; SOON kept)",
