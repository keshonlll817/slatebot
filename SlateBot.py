...
engage_state = {
    ...
    "review_last_sent": None, # datetime of the last review nudge
    "review_msg": None,       # [channel_id, message_id] of the live review nudge
    "review_next_channel": "A",
    "review_target_hour": None,
...
def _engage_save():
    try:
        blob = dict(engage_state)
        rl = blob.get("review_last_sent")
        blob["review_last_sent"] = rl.isoformat() if isinstance(rl, datetime) else None
...
def _engage_load():
    try:
        ...
        if rl:
            try:
                blob["review_last_sent"] = datetime.fromisoformat(rl)
            except Exception:
                blob["review_last_sent"] = None
        ...
        log.info(f"[ENGAGE] LOAD streak={engage_state['streak']} "
                 ...
                 f"review_next={engage_state['review_next_channel']}")
    except Exception as e:
...
async def _check_review():
    now = datetime.now(EST)
    last = engage_state.get("review_last_sent")
    if last and (now - last).total_seconds() < REVIEW_INTERVAL_DAYS * 86400:
        engage_state["review_target_hour"] = None   # re-roll for the next cycle
        return

    # Pick the hour ONCE per cycle and commit to it, rather than re-rolling
    # every poll (which made the actual send time unpredictable).
    target = engage_state.get("review_target_hour")
    if target is None:
        target = random.choice(REVIEW_HOUR_CHOICES)
        engage_state["review_target_hour"] = target
        _engage_save()
        log.info(f"[ENGAGE] REVIEW next nudge scheduled for {target}:00 EST")
    if now.hour != target:
        return

    # Exactly one live review nudge at a time.
    old = engage_state.get("review_msg")
    if old:
        ...
        if ch is not None:
            try:
                ...
                await msg.delete()
                log.info(f"[ENGAGE] REVIEW removed previous nudge {old[1]}")
            except discord.NotFound:
                pass
            except Exception as e:
                log.warning(f"[ENGAGE] REVIEW could not delete {old[1]} "
                            f"({type(e).__name__}: {e})")

    which = engage_state.get("review_next_channel", "A")
    target = REVIEW_CHANNEL_A if which == "A" else REVIEW_CHANNEL_B
    text = random.choice(REVIEW_MESSAGES).format(link=REVIEW_LINK)
    sent = await _engage_send(target, text)
    if sent is None:
        return
    engage_state["review_msg"] = [target, getattr(sent, "id", None)]
    engage_state["review_last_sent"] = now
    engage_state["review_next_channel"] = "B" if which == "A" else "A"
    _engage_save()
    log.info(f"[ENGAGE] REVIEW nudge sent to {target} at {now:%I:%M %p}; "
             f"next goes to {'B' if which=='A' else 'A'}")


# ==================================================================
# AUTOMATED RECAPS
# ==================================================================
# Posts the daily recap at the end of each day, plus weekly on Sunday
# and monthly on the last day of the month.
#
# The critical detail is WINDOW PINNING. "weekly" normally means
# "this Monday 00:00 → +7d" and "monthly" means "the 1st → now", both
# computed from the clock at send time. The daily post deliberately
# waits for late games to be graded, so it can drift past midnight —
# and a window computed at 12:30 AM Monday would return the brand-new
# (empty) week, or on the 1st the brand-new (empty) month.
#
# So every window here is derived from `anchor` — the date being
# recapped — captured when the sequence starts, never from "now".
# ==================================================================
