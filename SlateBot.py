...
def _resolve_slate_dates(raw_plays, anchor_dt):
    ...
    Rules:
    - The FIRST game in the message anchors the calendar date to the date
    ...
    - If the first game is more than 1 hour before the POST time, the slate
      was posted the night before for the next day — anchor to the day
      after posting.
    ...
    # First game >1h before the slate was POSTED → it was posted the night
    # before for the next day. Anchor to the day after posting.
    if first_game_dt < anchor_dt - timedelta(hours=1):
        anchor_date = (anchor_dt + timedelta(days=1)).date()
        try:
            naive_first = datetime.strptime(
                f"{anchor_date.year}/{anchor_date.month}/{anchor_date.day} {first_time_str}",
                "%Y/%m/%d %I:%M %p"
...
async def _schedule_message_plays_inner(message, text=None):
    ...
    if not raw_plays:
        ...
        # when "the bot stopped picking up games".
        if " vs " in text.lower() and re.search(r'\best\b', text, re.IGNORECASE):
