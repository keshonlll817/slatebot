...
# Pending reminder batches: {guild_id: {slot_key: [play_info]}}
# Used to group games starting within 5 min of each other into one message
_pending_soon  = {}   # {guild_id: {batch_key: [play_dicts]}}
_pending_now   = {}   # {guild_id: {batch_key: [play_dicts]}}
_batch_locks   = {}   # {guild_id: asyncio.Lock}


def _get_batch_lock(guild_id):
    if guild_id not in _batch_locks:
        _batch_locks[guild_id] = asyncio.Lock()
    return _batch_locks[guild_id]


def _batch_slot(game_dt, window_minutes=5):
    """Round game_dt down to nearest window to group nearby games."""
    slot = game_dt.replace(second=0, microsecond=0)
    slot_minutes = (slot.minute // window_minutes) * window_minutes
    return slot.replace(minute=slot_minutes)
...
async def _reminder_task(guild_id, key, guild, league, p1, p2, wins, total, tier, game_dt, reminder_channel_id, play_type="", stagger_seconds=0):
    """
    Single async task per play. Sends STARTING SOON and STARTING NOW alerts.
    Games starting within 5 minutes of each other are batched into one message.
    Fixes:
    - NOW always fires even if SOON was late (uses absolute game_dt target)
    - NOW fires even if game already started (30s grace window)
    - Stagger applied to both SOON and NOW
    """
    try:
        soon_dt = game_dt - timedelta(minutes=5)
        play_info = {
            "guild": guild, "league": league, "p1": p1, "p2": p2,
            "wins": wins, "total": total, "tier": tier,
            "play_type": play_type, "key": key,
        ...
        # Always attempt SOON send (even if slightly late — within 4 min grace)
        now_after_soon = datetime.now(EST)
        soon_grace = (now_after_soon - soon_dt).total_seconds()
        if soon_grace <= 240:  # within 4 min of when SOON should have fired
            if not locked or guild_id == TEST_GUILD_ID:
                await _send_batched_alert(play_info, "STARTING SOON", guild_id, stagger_seconds)

        # ── STARTING NOW ──
        # Always recalculate from real clock — never skip NOW due to SOON delay
        now_est2    = datetime.now(EST)
        start_delay = (game_dt - now_est2).total_seconds()

        if start_delay > 0:
            print(f"[REMINDERS] Sleeping {start_delay:.0f}s for NOW: {key}")
            await asyncio.sleep(start_delay)

        # Fire NOW even if slightly past game time (30s grace)
        now_after_now = datetime.now(EST)
        now_grace = (now_after_now - game_dt).total_seconds()
        if now_grace <= 30:
            if not locked or guild_id == TEST_GUILD_ID:
                await _send_batched_alert(play_info, "STARTING NOW", guild_id, stagger_seconds)
...
async def _send_batched_alert(play_info, label, guild_id, stagger_seconds=0):
    """
    Send a reminder alert. Games within a 5-min window are batched into one message.
    Uses a short collection window (2s) to gather simultaneous alerts.
    """
    reminder_channel_id = play_info["reminder_channel_id"]
    guild               = play_info["guild"]
    batch_key           = f"{reminder_channel_id}|{label}|{datetime.now(EST).strftime('%Y%m%d%H%M')[:-1]}"

    if guild_id not in _pending_soon:
        _pending_soon[guild_id] = {}
    if guild_id not in _pending_now:
        _pending_now[guild_id] = {}

    store = _pending_soon[guild_id] if label == "STARTING SOON" else _pending_now[guild_id]

    # Add this play to the batch
    if batch_key not in store:
        store[batch_key] = []
    store[batch_key].append(play_info)

    # Wait briefly to collect other plays firing at the same time
    await asyncio.sleep(2 + stagger_seconds * 0.1)

    # Only the first task to reach here sends — others find batch already sent
    if batch_key not in store:
        return
    batch = store.pop(batch_key, [])
    if not batch:
        return

    ch = await _fetch_ch_safe(reminder_channel_id)
    if not ch:
        print(f"[REMINDERS] Channel {reminder_channel_id} not found for {label}")
        return

    dest_guild = getattr(ch, "guild", guild)

    # Build combined message for all plays in batch
    lines = []
    for p in batch:
        line = build_reminder_text(
            dest_guild, p["league"], p["p1"], p["p2"],
            p["wins"], p["total"], p["tier"], label, p["play_type"]
        )
        lines.append(line)

    # For batched messages, only ping once at the top
    if len(lines) > 1:
        # Strip role mentions from individual lines, add one ping at top
        clean_lines = []
        for line in lines:
            clean_line = re.sub(r'<@&\d+>\s*', '', line).strip()
            clean_lines.append(clean_line)

        # Get the role mention from the first line build
        role_mention = ""
        first_full = lines[0]
        role_m = re.match(r'(<@&\d+>)', first_full)
        if role_m:
            role_mention = role_m.group(1) + " "

        text = role_mention + f"**{label}**\n" + "\n".join(clean_lines)
        mentions = _allowed_mentions_for_guild(dest_guild)
    else:
        text     = lines[0]
        mentions = _allowed_mentions_for_guild(dest_guild)

    await ch.send(text, allowed_mentions=mentions)
    print(f"[REMINDERS] Sent {label} ({len(batch)} play(s)): {[p['key'] for p in batch]}")
...
def _schedule_play_for_message(guild_id, message_id, guild, play, game_dt, reminder_channel_id):
    ...
    # Create the task
    # Stagger simultaneous reminders by 1.5s per play to avoid Discord rate limits
    task_count = sum(len(v) for v in scheduled_tasks.get(guild_id, {}).values())
    stagger = (task_count % 10) * 1.5

    task = asyncio.ensure_future(
        _reminder_task(guild_id, key, guild, league, p1, p2, wins, total, tier, game_dt, reminder_channel_id, play_type, stagger)
    )
