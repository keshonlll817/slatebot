...
def _destroy_to_dt(parsed, anchor_dt):
    ...
    dt = anchor_dt.replace(hour=hh % 24, minute=mm, second=0, microsecond=0)
    if dt < anchor_dt - timedelta(hours=1):
        dt += timedelta(days=1)
    return dt
...
def parse_destroy_message(text, anchor_dt):
    ...
    while i < len(lines):
        ...
        if not bare or _is_graded(bare):
            i += 1
            continue
        ...
        if hdr and not _DESTROY_PARLAY_RE.search(bare):
            ...
            current = {
                ...
                "bets": [], "game_dt": _destroy_to_dt(parsed, anchor_dt),
            }
            i += 1
            continue
...
async def _run_lixx_recap(sport_key, arg):
    ...
    color, icon = recap_color(totals["units"])
    embed = discord.Embed(title=f"\U0001f4ca LIXX — {scope} RECAP — {l_title}", color=color)
    ...
    if graded == 0:
        ...
        if totals["risked"] > 0:
            roi = totals["units"] / totals["risked"] * 100
            body += f"\n**Risked:** `{totals['risked']:.1f}U` ({roi:+.1f}% ROI)"
        embed.description = body

        # Per-sport split only when more than one sport is in scope
        if sport_key == "all":
            for label, st in sorted(per_sport, key=lambda x: -x[1]["units"]):
                if st["w"] + st["l"] + st["wash"] == 0:
                    continue
                s_icon = recap_color(st["units"])[1]
                line = f"{s_icon} `{st['w']}-{st['l']}`  `{st['units']:+.2f}U`"
                if st["risked"] > 0:
                    line += f"  ({st['units']/st['risked']*100:+.1f}% ROI)"
                embed.add_field(name=label, value=line, inline=True)

    foot = _format_ungraded_footer(ungraded, "play")
    if foot:
        embed.add_field(name="\u200b", value=foot.strip()[:1024], inline=False)
    if denied:
        embed.set_footer(text="Couldn\u2019t read: " + ", ".join(denied) +
...
def _was_recently_sent(guild_id, key, label, now):
    ts = recently_sent_alerts.get((guild_id, key, label))
    return ts is not None and (now - ts).total_seconds() < ALERT_RESEND_SUPPRESS_SECONDS
...
def _cancel_alerts_for_key(guild_id, key):
    ...
    MUST mutate the list IN PLACE. Rebinding pending_alerts[guild_id] to a
    new list caused a race that silently wiped freshly scheduled reminders:
    the dispatcher captures the list object, awaits ch.send() (a network
    call), and afterwards rewrites the registry from its captured
    reference. If an edit rebound the key to a NEW list during that await,
    every alert the edit had just registered was discarded on resume —
    while scheduled_tasks/active_keys kept the play, so the confirmation
    said "REMINDERS SET", !reminders showed nothing, and nothing fired
    until the slate was reposted.
    """
    alerts = pending_alerts.get(guild_id)
...
def _build_reminders_list(guild_id):
    ...
    for idx, (key, meta) in enumerate(sorted_plays, start=1):
        ...
        source = meta.get("source", "4+")
        if source == "destroy":
            # Destroy metadata stores the whole matchup in one field, so the
            # "{league} – {p1} vs {p2}" shape would render it doubled with a
            # dangling "vs". Use the prebuilt title instead.
            tag   = "\U0001f4a5"
            label = meta.get("display") or f"{p1_k}"
            row   = f"**{idx}.** {tag} {label} @ {time_k} EST ({countdown})"
            ...
            tag = "\U0001f3d3"
            row = (f"**{idx}.** {tag} {league_k} – {p1_k.title()} vs {p2_k.title()} "
                   f"@ {time_k} EST ({countdown})")
        lines.append(row)

    lines.append("\n\U0001f3d3 4+ plays  ·  \U0001f4a5 Destroy Plays")
...
async def _sweep_channel(channel, label):
    """Schedule any message in `channel` that we are not tracking at all.

    Message-level granularity keeps this additive: a slate we already track
    is left alone (its edits are handled by the edit handler), so the sweep
    never cancels a live reminder. It only picks up slates that fell out of
    memory entirely — the restart / lost-state case.
    """
    cutoff = datetime.now(EST) - timedelta(hours=RECOVERY_LOOKBACK_HOURS)
    added = 0
    ...
    async for msg in channel.history(limit=200):
        if msg.created_at.astimezone(EST) < cutoff:
            break
        gid = _guild_id(msg)
        if scheduled_tasks.get(gid, {}).get(msg.id):
            continue  # already tracked — leave it alone
        if is_destroy:
            if msg.author.bot:
                continue
            n = await schedule_destroy_plays(msg)
        else:
            n = len(await schedule_message_plays(msg))
        if n:
            added += n
            log.warning(f"[RECOVERY] {label}: message {msg.id} was untracked — "
                        f"scheduled {n} play(s).")
    return added


async def _recovery_sweep():
    """Re-read the slate channels and schedule anything not already tracked.

    Everything about reminders lives in memory, so a Railway deploy, a crash,
    or any future desync empties it — and until now the ONLY thing that
    rebuilt it was a single startup rescan. If that rescan raced a game or
    the process restarted close to a start time, the game was simply lost.

    This sweep re-derives from the messages themselves on a timer, so state
    can never drift for more than one interval. It is strictly additive:
    active_keys already blocks duplicates and _was_recently_sent blocks
    re-pings, so a play that is tracked or already alerted is untouched.
    """
    await client.wait_until_ready()
    await asyncio.sleep(60)  # let the startup rescan finish first
    while not client.is_closed():
        try:
            if locked:
            ...
            for ch_id, label in [(FOUR_PLUS_CHANNEL, "4+"),
                ...
                ch = await _fetch_ch_safe(ch_id)
                if ch is None:
                    log.warning(f"[RECOVERY] Could not resolve {label} channel {ch_id}")
                    continue
                try:
                    added += await _sweep_channel(ch, label)
                except Exception:
                    log.exception(f"[RECOVERY] Sweep of {label} failed (continuing):")
            restored = _reconcile_alerts()
            if added or restored:
                log.warning(f"[RECOVERY] Sweep recovered {added} untracked play(s) "
                            f"and restored {restored} missing alert(s).")
            else:
                log.info("[RECOVERY] Sweep clean — every play in the last "
                         f"{RECOVERY_LOOKBACK_HOURS}h is tracked.")
        except Exception:
            log.exception("[RECOVERY] Sweep loop error (continuing):")
        await asyncio.sleep(RECOVERY_INTERVAL_SECONDS)


def _reconcile_alerts():
    """Re-register alerts for scheduled plays that have lost them.

    Safety net: scheduled_tasks is the record of what SHOULD have reminders.
    If a play there still has a future SOON/NOW that is neither pending nor
    already sent, the alert went missing and is restored here. Runs each
    heartbeat so a game can miss at most one 5-minute window rather than
    being silently dropped for good.
    """
    now = datetime.now(EST)
    restored = 0
    for gid, msgs in scheduled_tasks.items():
        alerts = pending_alerts.get(gid, [])
        live = {(a["key"], a["label"]) for a in alerts if not a["sent"]}
        for _msg_id, metas in msgs.items():
            for key, meta in metas.items():
                game_dt = meta.get("game_dt")
                play_data = meta.get("play_data")
                ch_id = meta.get("reminder_channel_id")
                if not game_dt or not play_data or not ch_id:
                    continue  # pre-upgrade entry, nothing to rebuild from
                for label, fire_at in (("STARTING SOON", game_dt - timedelta(minutes=5)),
                                       ("STARTING NOW", game_dt)):
                    if (key, label) in live:
                        continue
                    if _was_recently_sent(gid, key, label, now):
                        continue
                    # Only restore alerts that can still legitimately fire
                    if (fire_at - now).total_seconds() <= -ALERT_LATE_GRACE_SECONDS:
                        continue
                    _register_alert(gid, fire_at, label, play_data, ch_id, key)
                    restored += 1
                    log.warning(f"[RECONCILE] Restored missing {label} for {key}")
    return restored


def _prune_finished_plays():
    """
    Housekeeping: drop metadata + dedup keys for plays whose game started
    more than 3 hours ago. Without this, scheduled_tasks/active_keys grow
    forever on long Railway uptimes. Returns number of plays pruned.
    """
    now = datetime.now(EST)
    horizon = now - timedelta(hours=3)
    pruned = 0

    # Expire sent-alert memory past the suppression window
    for k in list(recently_sent_alerts.keys()):
        if (now - recently_sent_alerts[k]).total_seconds() > ALERT_RESEND_SUPPRESS_SECONDS:
            del recently_sent_alerts[k]

    for gid in list(scheduled_tasks.keys()):
        for msg_id in list(scheduled_tasks.get(gid, {}).keys()):
            bucket = scheduled_tasks[gid][msg_id]
            for key in list(bucket.keys()):
                gdt = bucket[key].get("game_dt")
                if gdt and gdt < horizon:
                    del bucket[key]
                    active_keys.get(gid, set()).discard(key)
                    pruned += 1
            if not bucket:
                del scheduled_tasks[gid][msg_id]
    return pruned
...
async def _reminder_dispatcher_loop():
    """
    Single loop that runs forever. Every cycle it:
    ...
    Every ~5 minutes it logs a HEARTBEAT line and prunes finished plays.
    If heartbeats stop appearing in the Railway logs, the loop died and
    the supervisor will log the crash + restart it.
    """
    global _last_heartbeat
    await client.wait_until_ready()
    log.info("[DISPATCHER] Reminder dispatcher started.")

    while not client.is_closed():
        try:
            now = datetime.now(EST)

            # ── Heartbeat + housekeeping every 5 minutes ──
            if _last_heartbeat is None or (now - _last_heartbeat).total_seconds() >= 300:
                _last_heartbeat = now
                unsent = [a for alerts in pending_alerts.values() for a in alerts if not a["sent"]]
                nxt = min((a["fire_at"] for a in unsent), default=None)
                nxt_s = nxt.strftime("%m/%d %I:%M %p") if nxt else "none"
                keys_total = sum(len(s) for s in active_keys.values())
                log.info(f"[HEARTBEAT] dispatcher alive | pending alerts: {len(unsent)} | next fire: {nxt_s} | tracked keys: {keys_total}")
                pruned = _prune_finished_plays()
                if pruned:
                    log.info(f"[HOUSEKEEPING] Pruned {pruned} finished play(s) from registry.")
                restored = _reconcile_alerts()
                if restored:
                    log.warning(f"[RECONCILE] Restored {restored} missing alert(s) — investigate if this recurs.")
            ...
            for guild_id, alerts in list(pending_alerts.items()):
                ...
                for a in due:
                    if (now - a["fire_at"]).total_seconds() > 300:
                        a["sent"] = True
                        log.warning(f"[DISPATCHER] Skipped stale {a['label']} for {a['key']} ({int((now - a['fire_at']).total_seconds())}s late)")
                ...
                for (ch_id, label), group in groups.items():
                    ch = await _fetch_ch_safe(ch_id)
                    if not ch:
                        log.error(f"[DISPATCHER] Channel {ch_id} not found — will retry next cycle")
                    ...
                    for a in group:
                        p = a["play"]
                        if p.get("destroy"):
                            lines.append(build_destroy_text(dest_guild, p["entry"], label))
                        else:
                    ...
                    try:
                        ...
                        for a in group:
                            a["sent"] = True
                            recently_sent_alerts[(guild_id, a["key"], a["label"])] = now
                        log.info(f"[DISPATCHER] Sent {label} ({len(group)} play(s)) to ch={ch_id}: {', '.join(a['key'] for a in group)}")
                    except Exception as e:
                        log.error(f"[DISPATCHER] Send failed for {label} to ch={ch_id}: {e} — will retry")
                        # don't mark sent, retry next cycle

                # Clean up alerts sent or more than 10 min past their fire time.
                # Re-fetch and mutate IN PLACE so anything registered while we
                # were awaiting the sends above survives (see
                # _cancel_alerts_for_key for the race this prevents).
                live = pending_alerts.get(guild_id)
                if live is not None:
                    live[:] = [
                        a for a in live
            ...
            log.exception("[DISPATCHER] Loop iteration error (loop continues):")
...
def _schedule_play_for_message(guild_id, message_id, guild, play, game_dt, reminder_channel_id):
    ...
    key     = make_play_key(league, p1, p2, time_str, game_dt)
    now_est = datetime.now(EST)
    soon_dt = game_dt - timedelta(minutes=5)

    # Game started more than the grace window ago — nothing left to fire
    if (game_dt - now_est).total_seconds() <= -ALERT_LATE_GRACE_SECONDS:
        log.debug(f"[SCHEDULE] Skipped (alerts already past): {key}")
        return key, False

    # Cross-message duplicate check — this skip was previously SILENT,
    # which made dropped reminders impossible to diagnose from logs.
    if key in active_keys[guild_id]:
        log.info(f"[SCHEDULE] Skipped (duplicate — already scheduled this game date): {key}")
        return key, False
    ...
    # Register SOON alert if still in the future or within the late-grace
    # window — unless it already actually went out moments ago (edit /
    # delete-repost right after a send must not re-ping).
    if (soon_dt - now_est).total_seconds() > -ALERT_LATE_GRACE_SECONDS:
        if _was_recently_sent(guild_id, key, "STARTING SOON", now_est):
            log.info(f"[SCHEDULE] Suppressed re-send of STARTING SOON (already sent recently): {key}")
        else:
            _register_alert(guild_id, soon_dt, "STARTING SOON", play_data, reminder_channel_id, key)

    # Register NOW alert under the same rules
    if (game_dt - now_est).total_seconds() > -ALERT_LATE_GRACE_SECONDS:
        if _was_recently_sent(guild_id, key, "STARTING NOW", now_est):
            log.info(f"[SCHEDULE] Suppressed re-send of STARTING NOW (already sent recently): {key}")
        else:
    ...
    scheduled_tasks[guild_id][message_id][key] = {
        ...
        # Kept so _reconcile_alerts() can re-register a lost alert without
    ...
    live_labels = [a["label"] for a in pending_alerts.get(guild_id, [])
                   if a["key"] == key and not a["sent"]]
    if live_labels:
        log.info(f"[SCHEDULE] Scheduled: {key} @ {game_dt.strftime('%m/%d %I:%M %p')} EST "
                 f"(SOON {soon_dt.strftime('%I:%M %p')}) [{', '.join(sorted(live_labels))}]")
    else:
        log.info(f"[SCHEDULE] Tracked (no alerts left to fire): {key} @ {game_dt.strftime('%m/%d %I:%M %p')} EST")
    return key, True
...
def _cancel_message_tasks(guild_id, message_id):
    ...
    for key in msg_tasks.keys():
        ...
        log.info(f"[SCHEDULE] Cancelled: {key}")
...
async def schedule_destroy_plays(message, text=None):
    """Schedule reminders for a Destroy Plays slate.

    Registers into the SAME pending_alerts registry as the 4+ plays and
    targets the same reminder channel, so when a Destroy match and a 4+
    play start at the same minute the dispatcher groups them into one
    message and merges both role pings.
    """
    try:
        ...
        reminder_channel_id = TEST_GENERAL_CH if _is_test(message) else REMINDER_CHANNEL
        now_est = datetime.now(EST)
        scheduled, skipped_past, dupes = 0, 0, 0

        for e in entries:
            game_dt = e["game_dt"]
            key = make_destroy_key(e)
            soon_dt = game_dt - timedelta(minutes=5)
            ...
            if key in active_keys[guild_id]:
                dupes += 1
                log.info(f"[DESTROY] Skipped (duplicate): {key}")
                continue

            play_data = {"destroy": True, "entry": e}
            for label, fire_at in (("STARTING SOON", soon_dt), ("STARTING NOW", game_dt)):
                if (fire_at - now_est).total_seconds() <= -ALERT_LATE_GRACE_SECONDS:
                    continue
                if _was_recently_sent(guild_id, key, label, now_est):
                    log.info(f"[DESTROY] Suppressed re-send of {label}: {key}")
                    continue
                _register_alert(guild_id, fire_at, label, play_data, reminder_channel_id, key)

            scheduled_tasks[guild_id].setdefault(message_id, {})[key] = {
                "game_dt": game_dt,
                "source": "destroy",
                "display": e["title"],
                "bets": list(e.get("bets", [])),
                "league": e.get("league", "DESTROY"),
                "p1": e["title"], "p2": "",
            ...
            active_keys[guild_id].add(key)
            scheduled += 1
            log.info(f"[DESTROY] Scheduled: {key} @ {game_dt.strftime('%m/%d %I:%M %p')} EST")

        log.info(f"[DESTROY] Message {message_id} (posted {anchor.strftime('%m/%d %I:%M %p')}): "
                 f"{len(entries)} entr(ies) parsed → {scheduled} scheduled, "
                 f"{skipped_past} already started, {dupes} duplicate")
        return scheduled
    except Exception:
        log.exception(f"[DESTROY] Scheduling FAILED for message {getattr(message,'id','?')}:")
        return 0
...
async def schedule_message_plays(message, text=None):
    ...
    3. Resolve correct calendar dates for the whole slate as a batch,
       anchored on the time the message was POSTED (not the current time).
    4. Skip plays whose game start time is already in the past.
    5. Schedule alerts for each future play.
    6. Return list of result dicts for confirmation message.

    Wrapped in a guard so a single malformed message can never crash the
    on_message / startup-rescan flow — failures are logged with traceback.

    If text is None, uses message.content.
    """
    try:
        return await _schedule_message_plays_inner(message, text)
    except Exception:
        log.exception(f"[REMINDERS] schedule_message_plays FAILED for message {getattr(message, 'id', '?')}:")
        return []


async def _schedule_message_plays_inner(message, text=None):
    if text is None:
    ...
    if not raw_plays:
        # Diagnostic: message in a watched channel that LOOKS like a slate
        # but produced zero parseable plays — this is the #1 thing to check
        # when "the bot stopped picking up games". A slate where every line
        # is already graded (✅/❌/🧼) legitimately parses to 0, so only warn
        # if at least one UNGRADED line contains a matchup.
        ungraded_vs = any(
            " vs " in ln.lower() and not any(m in ln for m in ("✅", "❌", "🧼"))
            for ln in text.splitlines()
        )
        if ungraded_vs and re.search(r'\best\b', text, re.IGNORECASE):
            log.warning(f"[REMINDERS] Message {message_id} looks like a slate but parsed to 0 plays. First 150 chars: {text[:150]!r}")
        return []

    # Step 3: resolve calendar dates as a batch, anchored on POST time.
    # message.created_at is the original post time even on edits/rescans.
    anchor_dt = message.created_at.astimezone(EST)
    now_est   = datetime.now(EST)
    resolved  = _resolve_slate_dates(raw_plays, anchor_dt)

    # Step 4 & 5: skip already-started games + schedule the rest
    results      = []
    skipped_past = 0
    skipped_dup  = 0
    ...
    for entry in resolved:
        ...
        # ── FUTURE FILTER ──
        # Schedule if the game is upcoming OR started within the late-grace
        # window (a delete/repost or edit moments after start must still be
        # able to fire the STARTING NOW). Skip only if it started more than
        # ALERT_LATE_GRACE_SECONDS ago.
        if (game_dt - now_est).total_seconds() <= -ALERT_LATE_GRACE_SECONDS:
            skipped_past += 1
            log.debug(f"[SCHEDULE] Skipped (already started): {play['league']} – {play['p1']} vs {play['p2']} @ {time_str} (game_dt={game_dt.strftime('%m/%d %I:%M %p')})")
            continue
    ...
    log.info(
        f"[REMINDERS] Message {message_id} (posted {anchor_dt.strftime('%m/%d %I:%M %p')}): "
        f"{len(raw_plays)} play line(s) parsed → {len(results)} scheduled, "
        f"{skipped_past} already started, {skipped_dup} duplicate/past-alert"
    )
...
def clear_all_reminders():
    ...
    log.info("[STARTUP] Cleared all in-memory reminder state.")


async def reschedule_from_channel(channel, lookback_hours=12):
    if getattr(channel, "id", None) == DESTROY_CHANNEL_ID:
        return await _reschedule_destroy_from_channel(channel, lookback_hours)
    return await _reschedule_four_from_channel(channel, lookback_hours)


async def _reschedule_destroy_from_channel(channel, lookback_hours=12):
    """Startup rescan for the Destroy Plays channel."""
    ...
    try:
        async for msg in channel.history(limit=200):
            ...
            if msg.author.bot:
                continue
            scanned += 1
            count += await schedule_destroy_plays(msg)
    except Exception:
        log.exception("[STARTUP] Destroy rescan failed:")
    log.info(f"[STARTUP] Rescan #{getattr(channel,'name','destroy')}: {scanned} scanned, {count} rescheduled")
    return count


async def _reschedule_four_from_channel(channel, lookback_hours=12):
    """
    On startup: scan recent messages in a channel and reschedule any plays
    whose game time is still in the future.
    Only looks back lookback_hours to avoid pulling in old slates.
    Processes both bot and human messages.
    Returns the number of plays rescheduled.
    """
    cutoff = datetime.now(EST) - timedelta(hours=lookback_hours)
    scheduled = 0
    scanned = 0
    ...
    async for msg in channel.history(limit=200):
        ...
        if msg_time < cutoff:
            break
        scanned += 1
        results = await schedule_message_plays(msg)
        scheduled += len(results)

    log.info(f"[STARTUP] Rescan #{getattr(channel, 'name', channel.id)}: {scanned} message(s) scanned, {scheduled} play(s) rescheduled.")
    return scheduled
...
async def send_reminder_confirmation(results, override_channel=None):
    ...
    if override_channel:
        ...
        ch = client.get_channel(CONFIRMATION_CHANNEL)
    if not ch:
        log.error("[CONFIRM] Confirmation channel not found — REMINDERS SET message not posted.")
        return

    tier_tag = {"rocket": " 🚀", "nuke": " ☢️", "caution": " ⚠️", "normal": ""}
    ...
    try:
        ...
        log.error(f"[CONFIRM] Failed to send REMINDERS SET message: {e}")
...
async def on_raw_message_delete(payload):
    ...
    _cancel_message_tasks(guild_id, payload.message_id)
...
# ==============================
    ...
    if content.startswith("!lixx"):
        ...
        emb, ok = await _run_lixx_recap(sport, " ".join(parts))
        if not ok:
            await message.channel.send(
                "Usage: `!lixx [" + "|".join(LIXX_SPORTS) + "|all] "
                "[today|yesterday|weekly|lastweek|monthly|ytd|lifetime|<date>]`")
            return
        ...
        for idx in sorted(set(indexes)):
            ...
            for msg_id, msg_meta in list(scheduled_tasks.get(guild_id, {}).items()):
                if key in msg_meta:
                    del msg_meta[key]
                    break
            log.info(f"[SCHEDULE] Removed by user: {key}")
...
async def slash_lixx(interaction: discord.Interaction,
    ...
    await interaction.response.defer()
    emb, ok = await _run_lixx_recap(sport_key, arg)
    if not ok:
        await interaction.followup.send(
            "Couldn't read that period — try the dropdown, or a date like `Jan 12 2026`.",
            ephemeral=True)
        return
