...
REMINDER_CHANNEL          = 1536188504651075584   # dedicated reminders channel (auto-cleans)
TT_CHAT_CHANNEL           = 1442258139985608867   # #tabletennis-chat — hype/streaks/milestones
FREE_CHAT_CHANNEL         = 1467549729901645866   # free members chat
REVIEW_CHANNEL_A          = 1442258139985608867   # review nudge alternates A -> B -> A ...
REVIEW_CHANNEL_B          = 1488744217705709568
WINNING_WAGERS_CHANNEL    = 1442043894287171746   # referenced in the +10U nudge, not posted to
FREE_RECAP_MIRROR_CHANNEL = 1443366029110677514   # free-member copy of the 4+ recap
PREMIUM_ROLE_ID           = 1466227540309053503
FREE_ROLE_ID              = 1497638912405930116
MEMBERSHIP_LINK           = "https://whop.com/royaltywagers/royalty-wagers-premium/"
REVIEW_LINK               = "https://whop.com/royaltywagers/reviews"
MOD_CODES                 = ["13lessed", "Esky", "Lixx", "Dark"]   # 10% off any purchase

# ── Engagement thresholds (all configurable) ──
# Bucket recap plays by GAME time rather than by when the slate was posted,
# so overnight slates can go up before midnight without their games falling
# between two daily recaps. Set False to restore message-time bucketing.
RECAP_BUCKET_BY_GAME_TIME = True

DAILY_UNITS_ALERT        = 10.0    # +U on the day (4+ only) that triggers the wagers nudge
UNIT_SIZES_SHOWN         = [5, 10, 25]   # $ per unit examples rotated in that message
STREAK_MIN               = 5       # first tier; nothing announced below this
STREAK_TIERS             = [5, 6, 9, 12, 15]   # tier floors — announce only on crossing
MILESTONE_STEP           = 250.0   # YTD 4+ units per milestone
MILESTONE_CAP            = 2000.0  # highest milestone tracked
# Review nudges run on INDEPENDENT timers per channel: TT chat weekly,
# general every two weeks. So every other week both go out together, and
# on the off weeks only TT chat gets one.
REVIEW_TT_INTERVAL_DAYS  = 7
REVIEW_GEN_INTERVAL_DAYS = 14

# ╔══════════════════════════════════════════════════════════════╗
# ║  ONE-TIME KICKOFF — REMOVE BEFORE THE NEXT BUILD             ║
# ║  Sends a review nudge to BOTH channels on first boot to      ║
# ║  start the two timers, then the 1-week / 2-week cycles run   ║
# ║  normally. Self-disables (persisted) so a restart can't      ║
# ║  resend.                                                     ║
# ║  >>> DELETE REVIEW_KICKOFF and its call site next build.     ║
# ╚══════════════════════════════════════════════════════════════╝
REVIEW_KICKOFF           = True
REVIEW_HOUR_CHOICES      = [12, 13, 14, 15, 16, 17]   # on the hour, EST
...
LIXX_SPORTS = {
    ...
    "general": ("CFB", 1506366131953209574),   # general-locks — CFB picks
}
...
# Channels where the "bang" trigger responds. Previously it answered in
# EVERY channel the bot could see; this keeps it to the chat channels that
# actually want it (TT plus the other sports channels and LixX's general).
BANG_CHANNELS = {
    1442038502647926824,
    1442039186504024075,
    1442039328850055227,
    ...
    1537516220071284836,
    1491816099921264690,
    1506366131953209574,   # LixX general-locks
    1442258139985608867,   # reminders channel
}
...
async def _send_yearly_breakdown(message, year, four_only=False):
    ...
    if is_test:
        ...
        totals_ch = client.get_channel(TEST_TOTALS_CH)
    else:
...
#       "key", "guild_id", "message_id", "source",
...
_rem_added_total = 0           # lifetime count of reminders added, for sweep reporting
rem_misses = []                # recent games whose alerts never fired
rem_sent_log = {}              # {(guild_id, key, label): datetime} — survives record deletion
rem_pending_deletes = []       # [{channel_id, message_id, delete_at}] — NOW alerts awaiting cleanup
REM_SENT_MEMORY_HOURS = 12
...
# A "STARTING SOON" that lands seconds before tip is noise. Its grace now
# stops this far short of game time; past that the warning is pointless and
# the STARTING NOW alert covers it instead.
REM_SOON_CUTOFF    = 90        # seconds before game time that SOON stops firing
# Reminders live in their own channel now, so BOTH alerts are cleaned up this
# long after posting to keep it clear. (Previously only STARTING NOW was
# deleted, because reminders shared the chat channel and history mattered.)
REM_NOW_DELETE_MINUTES = 45
REM_DELETE_ALL_LABELS  = True
...
def _note_sent(guild_id, key, label, when=None):
    rem_sent_log[(guild_id, key, label)] = when or datetime.now(EST)


def _already_sent(guild_id, key, label):
    """True if this exact alert went out recently.

    Records can be destroyed and rebuilt (a delete+repost, or a redeploy
    followed by a recovery sweep). Without this the rebuilt record starts
    with clean flags and fires an alert that already went out.
    """
    ts = rem_sent_log.get((guild_id, key, label))
    return ts is not None and (datetime.now(EST) - ts).total_seconds() < REM_SENT_MEMORY_HOURS * 3600
...
def _rem_save():
    ...
    try:
        ...
        blob["__sent__"] = [
            [g, k, l, ts] for (g, k, l), ts in rem_sent_log.items()
        ]
        # Survives a redeploy so a restart can't orphan a message forever.
        blob["__deletes__"] = [dict(d) for d in rem_pending_deletes]
...
def _rem_load():
    ...
    try:
        ...
        for g, k, l, ts in blob.pop("__sent__", []):
            if isinstance(ts, datetime):
                rem_sent_log[(g, k, l)] = ts
        for d in blob.pop("__deletes__", []):
            if isinstance(d.get("delete_at"), datetime):
                rem_pending_deletes.append(d)
        if rem_pending_deletes:
            log.info(f"[REM] LOAD {len(rem_pending_deletes)} pending message deletion(s)")
        for gid, recs in blob.items():
            for k, r in recs.items():
                ...
                n += 1
        moved = 0
        for bucket in reminders.values():
            for r in bucket.values():
                want = TEST_GENERAL_CH if r.get("guild_id") == TEST_GUILD_ID else REMINDER_CHANNEL
                if r.get("dest_channel_id") != want:
                    r["dest_channel_id"] = want
                    moved += 1
        if moved:
            log.warning(f"[REM] LOAD re-pointed {moved} restored reminder(s) "
                        f"to the current reminder channel ({REMINDER_CHANNEL})")
            _rem_save()
...
def sync_reminders(guild_id, message_id, source, desired, dest_channel_id):
    ...
    for key, (game_dt, payload) in want.items():
        ...
        if existing is None:
            prior_soon = _already_sent(guild_id, key, "STARTING SOON")
            prior_now = _already_sent(guild_id, key, "STARTING NOW")
            bucket[key] = {
                ...
                "soon_sent": prior_soon, "now_sent": prior_now,
                "updated": now.isoformat(),
            }
            added += 1
            globals()["_rem_added_total"] = globals().get("_rem_added_total", 0) + 1
            if prior_soon or prior_now:
                fired = ", ".join(l for l, f in (("SOON", prior_soon), ("NOW", prior_now)) if f)
                log.info(f"[REM] =SENT {key} rebuilt; {fired} already went out — not re-pinging")
        ...
        if existing["message_id"] != message_id:
            ...
            kept += 1
            continue
        # A reminder created before REMINDER_CHANNEL changed keeps its old
        # destination forever unless it is re-pointed here — records restored
        # from the state file are exactly that case, which is why reminders
        # kept landing in the previous channel after the move.
        if existing.get("dest_channel_id") != dest_channel_id:
            log.warning(f"[REM] ~CHAN {key} destination "
                        f"{existing.get('dest_channel_id')} -> {dest_channel_id}")
            existing["dest_channel_id"] = dest_channel_id
            updated += 1
...
def _armed_seconds(rec, now):
    """Seconds since this reminder was created / last re-timed."""
    try:
        return (now - datetime.fromisoformat(rec["updated"])).total_seconds()
    except Exception:
        return 1e9
...
async def _process_pending_deletes(now):
    """Delete reminder messages whose retention window has elapsed."""
    ready = [d for d in rem_pending_deletes if d["delete_at"] <= now]
    if not ready:
        return
    for d in ready:
        rem_pending_deletes.remove(d)
        ch = await _fetch_ch_safe(d["channel_id"])
        if ch is None:
            log.warning(f"[REM] DELETE skipped — channel {d['channel_id']} unreachable")
            continue
        try:
            msg = await ch.fetch_message(d["message_id"])
            await msg.delete()
            log.info(f"[REM] DELETE removed reminder msg {d['message_id']} "
                     f"({REM_NOW_DELETE_MINUTES}m after posting)")
        except discord.NotFound:
            log.info(f"[REM] DELETE msg {d['message_id']} already gone — nothing to do")
        except discord.Forbidden:
            log.error(f"[REM] DELETE forbidden for msg {d['message_id']} — "
                      f"bot needs permission to delete its own messages in that channel")
        except Exception as e:
            log.warning(f"[REM] DELETE failed for msg {d['message_id']} "
                        f"({type(e).__name__}: {e})")
    _rem_save()
...
def _rem_prune():
    ...
    for gid, bucket in reminders.items():
        for k in [k for k, r in bucket.items() if r["game_dt"] < cutoff]:
            del bucket[k]
            n += 1
    old = [kk for kk, ts in rem_sent_log.items()
           if (now - ts).total_seconds() > REM_SENT_MEMORY_HOURS * 3600]
    for kk in old:
        del rem_sent_log[kk]
    if n:
...
def _build_reminders_list(guild_id):
    ...
    for idx, (key, r) in enumerate(plays, 1):
        ...
        p = r["payload"]
        lines.append(f"**{idx}.** 🏓 {p['league']} – {p['p1'].title()} vs {p['p2'].title()} "
                     f"@ {t} EST ({countdown})")
    ...
    while not client.is_closed():
        try:
            ...
            for gid, bucket in list(reminders.items()):
                for r in list(bucket.values()):
                    ...
                    if not r["soon_sent"]:
                        ...
                        to_tip = (r["game_dt"] - now).total_seconds()
                        if age >= 0 and to_tip > REM_SOON_CUTOFF:
                            due.append((gid, r, "STARTING SOON", "soon_sent"))
                        elif age >= 0:
                            # Too close to tip for a "starting soon" to mean
                            # anything. Suppress it; STARTING NOW still fires.
                            r["soon_sent"] = True
                            armed = _armed_seconds(r, now)
                            if armed > REM_SOON_MINUTES * 60 + 60:
                                # We had a full window and still didn't send it.
                                log.warning(f"[REM] STALE {r['key']} SOON skipped "
                                            f"({int(age)}s late, {int(to_tip)}s to tip)")
                                _record_miss(r, "STARTING SOON", age)
                            else:
                                log.info(f"[REM] SKIP  {r['key']} SOON — only {int(to_tip)}s "
                                         f"to tip (scheduled {int(armed)}s ago); NOW will still fire")
                    if not r["now_sent"]:
            ...
            for (gid, ch_id, label), items in groups.items():
                ...
                try:
                    sent_msg = await ch.send(text, allowed_mentions=_allowed_mentions_for_guild(guild))
                    for r, flag in items:
                        r[flag] = True
                        _note_sent(gid, r["key"], label)
                    log.info(f"[REM] SENT  {label} x{len(items)} -> ch={ch_id}: "
                             + ", ".join(r["key"] for r, _f in items))
                    # Only STARTING NOW is cleaned up. STARTING SOON stays by
                    # design. A grouped NOW message covers games that share a
                    # start time, so one timer from POST time retires the whole
                    # message — no per-key bookkeeping needed.
                    if (REM_DELETE_ALL_LABELS or label == "STARTING NOW") and getattr(sent_msg, "id", None):
                        rem_pending_deletes.append({
                            "channel_id": ch_id,
                            "message_id": sent_msg.id,
                            "delete_at": now + timedelta(minutes=REM_NOW_DELETE_MINUTES),
                            "keys": [r["key"] for r, _f in items],
                        })
                        log.info(f"[REM] QUEUE delete of {label} msg {sent_msg.id} in "
                                 f"{REM_NOW_DELETE_MINUTES}m")
                except Exception as e:
            ...
            await _process_pending_deletes(now)
...
# ENGAGEMENT ENGINE
# ==================================================================
# Community-facing automation: win streaks, a daily units nudge, YTD
# milestones, and a recurring review reminder.
#
# Every one of these posts publicly, so the guiding rule is: fire once,
# fire late rather than wrong, and never re-fire on a restart. State is
# persisted alongside the reminder state for exactly that reason.
# ==================================================================

engage_state = {
    "streak": 0,              # current consecutive 4+ wins
    "streak_tier": 0,         # highest tier already announced for this streak
    "last_units_alert": "",   # YYYY-MM-DD the +10U nudge last fired
    "milestone_passed": 0.0,  # highest YTD milestone already announced
    "review_tt_last": None,   # datetime of the last TT-chat nudge
    "review_tt_msg": None,    # [channel_id, message_id] currently live there
    "review_tt_hour": None,   # hour chosen for the upcoming TT nudge
    "review_gen_last": None,  # same three, for the general channel
    "review_gen_msg": None,
    "review_gen_hour": None,
    "review_kickoff_done": False,   # ONE-TIME KICKOFF — remove next build
    "initialized": False,     # baseline captured yet?
}

ENGAGE_STATE_PATH = (os.path.join(os.path.dirname(REMINDER_STATE_PATH), "engagement.json")
                     if os.path.dirname(REMINDER_STATE_PATH) else "engagement.json")


def _engage_save():
    try:
        # Duck-typed rather than isinstance(v, datetime): safer against a
        # datetime subclass and against date objects sneaking in.
        blob = {k: (v.isoformat() if hasattr(v, "isoformat") else v)
                for k, v in engage_state.items()}
        tmp = ENGAGE_STATE_PATH + ".tmp"
        os.makedirs(os.path.dirname(ENGAGE_STATE_PATH) or ".", exist_ok=True)
        with open(tmp, "w") as f:
            json.dump(blob, f)
        os.replace(tmp, ENGAGE_STATE_PATH)
    except Exception as e:
        log.warning(f"[ENGAGE] SAVE failed ({type(e).__name__}: {e}) — running from memory")


def _engage_load():
    try:
        if not os.path.exists(ENGAGE_STATE_PATH):
            log.info(f"[ENGAGE] LOAD no state file at {ENGAGE_STATE_PATH} — first run")
            return
        with open(ENGAGE_STATE_PATH) as f:
            blob = json.load(f)
        for k in ("review_tt_last", "review_gen_last"):
            v = blob.get(k)
            if isinstance(v, str):
                try:
                    blob[k] = datetime.fromisoformat(v)
                except Exception:
                    blob[k] = None
        engage_state.update(blob)
        log.info(f"[ENGAGE] LOAD streak={engage_state['streak']} "
                 f"milestone_passed={engage_state['milestone_passed']}U "
                 f"tt_last={engage_state.get('review_tt_last')} "
                 f"gen_last={engage_state.get('review_gen_last')}")
    except Exception as e:
        log.warning(f"[ENGAGE] LOAD failed ({type(e).__name__}: {e}) — starting fresh")


# ── message pools ────────────────────────────────────────────────
STREAK_MESSAGES = {
    5:  ["🔥 **5 IN A ROW.** The board is cooperating.",
         "🔥 **5 straight.** Quietly heating up.",
         "🔥 **5-0 run going.** Locked in."],
    6:  ["🔥🔥 **{n} STRAIGHT.** This is a proper run now.",
         "🔥🔥 **{n} in a row** and the card keeps cashing.",
         "🔥🔥 **{n} straight winners.** Momentum is real."],
    9:  ["🚨 **{n} IN A ROW.** This is getting loud.",
         "🚨 **{n} STRAIGHT.** Somebody check on the books.",
         "🚨 **{n}-0.** The run is officially out of hand."],
    12: ["💥 **{n} STRAIGHT WINNERS.** Absolutely nuclear.",
         "💥 **{n} IN A ROW.** This is a heater for the record books.",
         "💥 **{n}-0 RUN.** Unreal scenes."],
    15: ["👑 **{n} IN A ROW.** ROYALTY BEHAVIOR. 👑",
         "👑 **{n} STRAIGHT.** This is what the crown looks like. 👑",
         "👑 **{n}-0.** Legendary run. Frame it. 👑"],
}

STREAK_END_MESSAGES = [
    "The **{n}-game run** ends there. Still a hell of a stretch. 🫡",
    "Streak snapped at **{n}**. On to the next one. 💪",
    "**{n} in a row** before that one. We'll take it every time. 🫡",
    "That ends the **{n}-game heater**. Regroup and run it back. 🔁",
]

# Two pools: DOLLAR messages quote a real $ figure (a unit size is rotated
# in), FLAT messages don't mention dollars at all. The bot picks a pool at
# random each day so the nudge doesn't always read the same way.
UNITS_ALERT_DOLLAR_MESSAGES = [
    "📈 **+{u:.2f}U on the day!** A **${size}/unit** bettor is up **${dollars:,.2f}** today.\n"
    "Drop your winning slips in <#{ch}> — let's see them! 🎟️",
    "🔥 **{u:.2f} units up today.** That's **${dollars:,.2f}** on a **${size} unit**.\n"
    "Post those wins in <#{ch}>, don't be shy! 💸",
    "💰 **Day's running +{u:.2f}U.** At **${size}/unit** that's **${dollars:,.2f}** in profit.\n"
    "Show it off in <#{ch}> 📸",
    "🏓 **+{u:.2f}U and counting.** **${size}** units → **${dollars:,.2f}** today.\n"
    "Cash tickets belong in <#{ch}> — let's fill it up! 🚀",
    "📊 **+{u:.2f}U today.** A **${size}/unit** player banks **${dollars:,.2f}**.\n"
    "Tag your slips in <#{ch}> 🏆",
]

UNITS_ALERT_FLAT_MESSAGES = [
    "✅ **{u:.2f}U day so far.** Head to <#{ch}> and post those winners — "
    "seeing them keeps everyone locked in. 🙌",
    "🚨 **+{u:.2f}U on the day.** The board is paying. "
    "Get your slips up in <#{ch}> 📸",
    "🏓 **{u:.2f} units up.** Good day to be on. "
    "Show the room what you hit in <#{ch}> 👀",
    "📈 **Day's sitting at +{u:.2f}U.** Don't let these go undocumented — "
    "<#{ch}> is waiting. 🎟️",
    "🔥 **+{u:.2f}U and the day isn't over.** "
    "Post 'em in <#{ch}> — the wall of wins doesn't build itself. 🧱",
    "💪 **{u:.2f}U in the green today.** If you tailed, "
    "<#{ch}> wants to see it. 🏆",
    "👑 **+{u:.2f}U on the day.** Royalty behavior. "
    "Slips go in <#{ch}> 💎",
    "🎯 **{u:.2f}U up.** Everyone posting in <#{ch}> right now is eating. "
    "Join them. 🍽️",
    "⚡ **+{u:.2f}U today.** Screenshots or it didn't happen — "
    "<#{ch}> 📲",
    "🙌 **{u:.2f}U day.** Nothing motivates the room like seeing real tickets. "
    "Drop yours in <#{ch}>",
]

MILESTONE_PREMIUM_MESSAGES = [
    "👑 **{m:,.0f} UNITS ON THE YEAR.** {role}\n"
    "You've been here for it. Thank you for riding with us. 🏓",
    "🏆 **{m:,.0f}U MILESTONE HIT.** {role}\n"
    "Another marker down. Onto the next one. 📈",
    "💎 **{m:,.0f} UNITS YTD.** {role}\n"
    "Consistency over everything. Appreciate every one of you. 🙌",
]

MILESTONE_FREE_MESSAGES = [
    "🚨 **{m:,.0f} UNITS ON THE YEAR** for Royalty Wagers. {role}\n"
    "That's the premium side of the house. Want in?\n"
    "{link}\n"
    "💸 Use any mod's code for **10% off**: {codes}",
    "📈 **We just crossed {m:,.0f} UNITS YTD.** {role}\n"
    "Imagine being on every one of those. You still can:\n"
    "{link}\n"
    "💸 **10% off** with any mod code: {codes}",
    "👑 **{m:,.0f}U ON THE YEAR.** {role}\n"
    "The free side sees a slice — members see it all.\n"
    "{link}\n"
    "💸 Grab **10% off** using a mod code: {codes}",
]

REVIEW_MESSAGES = [
    "⭐ **Enjoying the plays?** A quick review genuinely helps us grow — "
    "and takes about 30 seconds.\n{link}",
    "🙏 **Quick favor:** if the plays have treated you well, drop us a review.\n{link}",
    "⭐ **Got a minute?** Reviews are the biggest thing that helps new members "
    "find us. Would mean a lot.\n{link}",
    "💬 **How's it going so far?** Good or bad, we'd love to hear it — "
    "leave a review below.\n{link}",
    "🏓 **Been cashing with us?** Let people know — a quick review goes a long way.\n{link}",
    "⭐ **Shameless ask:** if you've been enjoying the slate, a review helps "
...
# ── helpers ──────────────────────────────────────────────────────
def _streak_tier(n):
    """Highest tier floor this streak has reached, or 0 if below minimum."""
    tier = 0
    for floor in STREAK_TIERS:
        if n >= floor:
            tier = floor
    return tier


async def _engage_send(channel_id, text, allowed=None):
    ch = await _fetch_ch_safe(channel_id)
    if ch is None:
        log.error(f"[ENGAGE] Channel {channel_id} unreachable — message dropped")
        return None
    try:
        return await ch.send(text, allowed_mentions=allowed or discord.AllowedMentions.none())
    except Exception as e:
        log.error(f"[ENGAGE] Send to {channel_id} failed ({type(e).__name__}: {e})")
        return None


async def _today_four_stats(limit=None):
    """Today's 4+ record and net units (EST day)."""
    now = datetime.now(EST)
    start = now.replace(hour=0, minute=0, second=0, microsecond=0)
    ch = await _fetch_ch_safe(FOUR_PLUS_CHANNEL)
    if ch is None:
        return None
    r = await parse_four_plus(ch, start, now, limit)
    fw, fl, fwash, nw, nl, cw, cl, kw, kl, rw, rl, _lg = r
    return {"w": fw, "l": fl, "wash": fwash,
            "units": four_plus_units(nw, nl, cw, cl, kw, kl, rw, rl)}


async def _ytd_four_units():
    now = datetime.now(EST)
    start = now.replace(month=1, day=1, hour=0, minute=0, second=0, microsecond=0)
    ch = await _fetch_ch_safe(FOUR_PLUS_CHANNEL)
    if ch is None:
        return None
    r = await parse_four_plus(ch, start, now, None)
    _fw, _fl, _fwash, nw, nl, cw, cl, kw, kl, rw, rl, _lg = r
    return four_plus_units(nw, nl, cw, cl, kw, kl, rw, rl)


async def _current_win_streak():
    """Consecutive 4+ wins, newest first. Washes are ignored entirely."""
    ch = await _fetch_ch_safe(FOUR_PLUS_CHANNEL)
    if ch is None:
        return None
    now = datetime.now(EST)
    ...
    try:
        async for msg in ch.history(limit=200):
            if (now - msg.created_at.astimezone(EST)).days > 14:
                break
            for raw in msg.content.split("\n"):
                ...
                if not ("✅" in line or "❌" in line or "🧼" in line):
                    continue
                dk = (msg.created_at.astimezone(EST).date(), line.replace("\ufe0f", ""))
                if dk in seen:
                    continue
                seen.add(dk)
                ordered.append(line)
    except Exception:
        log.exception("[ENGAGE] Streak scan failed:")
        return None
    streak = 0
    for line in ordered:           # newest first
        if "🧼" in line:
            continue               # wash: not a game, doesn't break or count
        if "✅" in line:
            streak += 1
        else:
            break
...
async def _engagement_loop():
    """Checks streaks, the daily units nudge, milestones, and reviews."""
    await client.wait_until_ready()
    await asyncio.sleep(90)
    _engage_load()

    # Baseline on first ever run so nothing historical fires retroactively.
    if not engage_state.get("initialized"):
        ytd = await _ytd_four_units()
        if ytd is not None:
            passed = min(math.floor(ytd / MILESTONE_STEP) * MILESTONE_STEP, MILESTONE_CAP)
            engage_state["milestone_passed"] = passed
            streak = await _current_win_streak()
            if streak is not None:
                engage_state["streak"] = streak
                engage_state["streak_tier"] = _streak_tier(streak)
            engage_state["initialized"] = True
            _engage_save()
            log.warning(f"[ENGAGE] BASELINE captured — YTD {ytd:+.2f}U, "
                        f"milestones up to {passed:,.0f}U marked as already passed, "
                        f"streak={engage_state['streak']}. Next milestone: "
                        f"{min(passed + MILESTONE_STEP, MILESTONE_CAP):,.0f}U")

    # ONE-TIME KICKOFF — remove this block with REVIEW_KICKOFF
    if REVIEW_KICKOFF and not engage_state.get("review_kickoff_done"):
        try:
            await _review_kickoff()
        except Exception:
            log.exception("[ENGAGE] KICKOFF failed:")

    while not client.is_closed():
        try:
            if not locked:
                await _check_streak()
                await _check_daily_units()
                await _check_milestone()
                await _check_review()
        except Exception:
            log.exception("[ENGAGE] Loop error (continuing):")
...
async def _check_streak():
    ...
    if streak is None:
        return
    prev, prev_tier = engage_state["streak"], engage_state["streak_tier"]
    if streak == prev:
        return

    if streak < prev and prev >= STREAK_MIN:
        text = random.choice(STREAK_END_MESSAGES).format(n=prev)
        await _engage_send(TT_CHAT_CHANNEL, text)
        log.info(f"[ENGAGE] STREAK ended at {prev}")

    tier = _streak_tier(streak)
    if tier and tier > prev_tier and streak > prev:
        text = random.choice(STREAK_MESSAGES[tier]).format(n=streak)
        await _engage_send(TT_CHAT_CHANNEL, text)
        log.info(f"[ENGAGE] STREAK tier {tier} announced at {streak} wins")
        engage_state["streak_tier"] = tier
    elif streak < prev:
        engage_state["streak_tier"] = _streak_tier(streak)
...
async def _check_daily_units():
    today = datetime.now(EST).strftime("%Y-%m-%d")
    if engage_state.get("last_units_alert") == today:
        return
    stats = await _today_four_stats()
    if stats is None or stats["units"] < DAILY_UNITS_ALERT:
        return
    # Weighted toward the flat pool so the "$X/unit" framing stays an
    # occasional highlight rather than the default voice.
    if random.random() < 0.35:
        size = random.choice(UNIT_SIZES_SHOWN)
        template = random.choice(UNITS_ALERT_DOLLAR_MESSAGES)
    else:
        size = 0
        template = random.choice(UNITS_ALERT_FLAT_MESSAGES)
    text = template.format(
        u=stats["units"], size=size, dollars=stats["units"] * size,
        ch=WINNING_WAGERS_CHANNEL)
    if await _engage_send(TT_CHAT_CHANNEL, text):
        engage_state["last_units_alert"] = today
        _engage_save()
        log.info(f"[ENGAGE] UNITS alert fired at {stats['units']:+.2f}U")
...
async def _check_milestone():
    ...
    if ytd is None:
        return
    passed = engage_state.get("milestone_passed", 0.0)
    nxt = passed + MILESTONE_STEP
    if nxt > MILESTONE_CAP or ytd < nxt:
        return
    reached = min(math.floor(ytd / MILESTONE_STEP) * MILESTONE_STEP, MILESTONE_CAP)

    prem = random.choice(MILESTONE_PREMIUM_MESSAGES).format(
        m=reached, role=f"<@&{PREMIUM_ROLE_ID}>")
    await _engage_send(TT_CHAT_CHANNEL, prem,
                       discord.AllowedMentions(roles=True))

    free = random.choice(MILESTONE_FREE_MESSAGES).format(
        m=reached, role=f"<@&{FREE_ROLE_ID}>", link=MEMBERSHIP_LINK,
        codes=" · ".join(f"`{c}`" for c in MOD_CODES))
    await _engage_send(FREE_CHAT_CHANNEL, free,
                       discord.AllowedMentions(roles=True))

    engage_state["milestone_passed"] = reached
    _engage_save()
    log.warning(f"[ENGAGE] MILESTONE {reached:,.0f}U announced (YTD {ytd:+.2f}U)")


async def _review_once(name, channel_id, interval_days, last_key, msg_key,
                       hour_key, force=False):
    """Post one review nudge if its own timer is due.

    Each channel runs an independent timer, so TT chat (weekly) and general
    (fortnightly) coincide every other week and TT goes alone in between.
    Exactly one nudge stays live per channel — the previous one is deleted
    before the new one is sent.
    """
    now = datetime.now(EST)
    last = engage_state.get(last_key)
    ...
    if not force:
        if last and (now - last).total_seconds() < interval_days * 86400:
            engage_state[hour_key] = None      # re-roll for the next cycle
            return False
        # Pick the hour once per cycle and commit, so the send time is
        # predictable instead of re-rolling on every poll.
        target = engage_state.get(hour_key)
        if target is None:
            target = random.choice(REVIEW_HOUR_CHOICES)
            engage_state[hour_key] = target
            _engage_save()
            log.info(f"[ENGAGE] REVIEW ({name}) next nudge set for {target}:00 EST")
        if now.hour != target:
            return False

    old = engage_state.get(msg_key)
    if old:
        ch = await _fetch_ch_safe(old[0])
        if ch is not None:
            try:
                msg = await ch.fetch_message(old[1])
                await msg.delete()
                log.info(f"[ENGAGE] REVIEW ({name}) removed previous nudge {old[1]}")
            except discord.NotFound:
                pass
            except Exception as e:
                log.warning(f"[ENGAGE] REVIEW ({name}) could not delete {old[1]} "
                            f"({type(e).__name__}: {e})")

    sent = await _engage_send(channel_id, random.choice(REVIEW_MESSAGES).format(link=REVIEW_LINK))
    if sent is None:
        return False
    engage_state[msg_key] = [channel_id, getattr(sent, "id", None)]
    engage_state[last_key] = now
    engage_state[hour_key] = None
    _engage_save()
    log.info(f"[ENGAGE] REVIEW ({name}) sent to {channel_id} at {now:%I:%M %p}; "
             f"next in {interval_days}d")
    return True


async def _check_review():
    await _review_once("tt-chat", REVIEW_CHANNEL_A, REVIEW_TT_INTERVAL_DAYS,
                       "review_tt_last", "review_tt_msg", "review_tt_hour")
    await _review_once("general", REVIEW_CHANNEL_B, REVIEW_GEN_INTERVAL_DAYS,
                       "review_gen_last", "review_gen_msg", "review_gen_hour")


async def _review_kickoff():
    """ONE-TIME: seed both timers by posting immediately. REMOVE next build."""
    engage_state["review_kickoff_done"] = True
    _engage_save()
    await _review_once("tt-chat", REVIEW_CHANNEL_A, REVIEW_TT_INTERVAL_DAYS,
                       "review_tt_last", "review_tt_msg", "review_tt_hour", force=True)
    await _review_once("general", REVIEW_CHANNEL_B, REVIEW_GEN_INTERVAL_DAYS,
                       "review_gen_last", "review_gen_msg", "review_gen_hour", force=True)
    log.warning(f"[ENGAGE] KICKOFF review nudges sent to both channels. "
                f"TT chat repeats in {REVIEW_TT_INTERVAL_DAYS}d, "
                f"general in {REVIEW_GEN_INTERVAL_DAYS}d.")


# ==================================================================
# AUTOMATED RECAPS
# ==================================================================
# Daily at the end of each day, plus weekly on Sunday and monthly on
# the last day of the month.
#
# WINDOW PINNING: every window is derived from `anchor` — the date being
# recapped, captured when the sequence starts — never from "now". The
# daily post deliberately waits for late grading and can drift past
# midnight; a window computed at 12:30 AM Monday would otherwise return
# the brand-new (empty) week, or on the 1st the brand-new (empty) month.
# ==================================================================

AUTO_RECAP_HOUR   = 23      # 11 PM EST
AUTO_RECAP_MINUTE = 59
AUTO_RECAP_CUTOFF_HOUR = 2  # give up waiting for grading at 2 AM and post anyway
# Until this hour the loop is still finishing YESTERDAY. Keeping it late
# means a night the bot was down or deployed late can still be caught up
# the next morning instead of that day's recap being lost for good.
AUTO_RECAP_CATCHUP_UNTIL_HOUR = 11


AUTO_RECAP_ENABLED = True

auto_recap_state = {"last_daily": "", "last_weekly": "", "last_monthly": ""}

AUTO_RECAP_STATE_PATH = (os.path.join(os.path.dirname(REMINDER_STATE_PATH), "autorecap.json")
                         if os.path.dirname(REMINDER_STATE_PATH) else "autorecap.json")


def _auto_recap_save():
    try:
        tmp = AUTO_RECAP_STATE_PATH + ".tmp"
        os.makedirs(os.path.dirname(AUTO_RECAP_STATE_PATH) or ".", exist_ok=True)
        with open(tmp, "w") as f:
            json.dump(auto_recap_state, f)
        os.replace(tmp, AUTO_RECAP_STATE_PATH)
    except Exception as e:
        log.warning(f"[AUTORECAP] SAVE failed ({type(e).__name__}: {e})")


def _auto_recap_load():
    try:
        if os.path.exists(AUTO_RECAP_STATE_PATH):
            with open(AUTO_RECAP_STATE_PATH) as f:
                auto_recap_state.update(json.load(f))
            log.info(f"[AUTORECAP] LOAD {auto_recap_state}")
    except Exception as e:
        log.warning(f"[AUTORECAP] LOAD failed ({type(e).__name__}: {e})")


def _window_for(kind, anchor):
    """(start, end, title) for a recap of `anchor`'s day/week/month.

    anchor is the DATE BEING RECAPPED, not the current time — see the
    module note above.
    """
    day0 = anchor.replace(hour=0, minute=0, second=0, microsecond=0)
    if kind == "daily":
        return day0, day0 + timedelta(days=1), f"DAILY RECAP — {anchor.strftime('%b %-d, %Y')} (EST)"
    if kind == "weekly":
        wk0 = day0 - timedelta(days=day0.weekday())          # Monday of anchor's week
        wk1 = wk0 + timedelta(days=7)
        return wk0, wk1, (f"WEEKLY RECAP — {wk0.strftime('%b %-d')} → "
                          f"{(wk1 - timedelta(days=1)).strftime('%b %-d')} (EST)")
    if kind == "monthly":
        m0 = day0.replace(day=1)
        m1 = (m0.replace(year=m0.year + 1, month=1) if m0.month == 12
              else m0.replace(month=m0.month + 1))
        return m0, m1, f"MONTHLY RECAP — {m0.strftime('%B %Y')}"
...
async def build_recap_text(start, end, title, four_channel, totals_channel, limit=None):
    """Recap body identical to the !recap output, for reuse by automation."""
    (fw, fl, fwash, nw, nl, cw, cl, kw, kl, rw, rl,
     league_stats) = await parse_four_plus(four_channel, start, end, limit)
    ...
    if league_stats:
        recap += "\n🏓 **LEAGUE BREAKDOWN**\n━━━━━━━━━━━━━━━━━━\n\n"
        for i, (lg, d) in enumerate(sorted(league_stats.items(),
                                           key=lambda x: x[1]["u"], reverse=True)):
            icon = ["🔥", "🟢", "🟡"][i] if i < 3 else "🔻"
            recap += f"{icon} {lg}\nRecord: {d['w']}-{d['l']}\nUnits: {d['u']:+.2f}U\n\n"
    return recap, (fw + fl + fwash > 0 or tw + tl > 0)


async def mirror_four_recap(recap_data):
    """Send the 4+ portion of a recap to the free-member recap channel.

    Identical to what the 4+ channel gets — same embeds, same content.
    """
    try:
        four = recap_data.get("four_plus") or {}
        if four.get("w", 0) + four.get("l", 0) + four.get("wash", 0) == 0:
            return False
        ch = await _fetch_ch_safe(FREE_RECAP_MIRROR_CHANNEL)
        if ch is None:
            log.error(f"[MIRROR] Cannot SEE channel {FREE_RECAP_MIRROR_CHANNEL} — "
                      f"SlateBot needs View Channel there (or the ID is wrong)")
            return False
        four_only = {
        ...
        embed, color = build_recap_embed(four_only)
        league_embed = build_league_embed(four_only, color)
        try:
            await ch.send(embed=embed)
            if league_embed:
                await ch.send(embed=league_embed)
        except discord.Forbidden:
            log.error(f"[MIRROR] FORBIDDEN posting to #{getattr(ch,'name','?')} "
                      f"({FREE_RECAP_MIRROR_CHANNEL}) — SlateBot needs Send Messages "
                      f"and Embed Links there")
            return False
        log.warning(f"[MIRROR] 4+ recap copied to #{getattr(ch,'name','free')}: "
                    f"{recap_data['title']}")
        return True
    except Exception:
        log.exception("[MIRROR] Failed to mirror 4+ recap:")
        return False


async def _post_auto_recap(kind, anchor):
    """Build and post one recap, then mirror the 4+ half."""
    start, end, title = _window_for(kind, anchor)
    four_ch = await _fetch_ch_safe(FOUR_PLUS_CHANNEL)
    totals_ch = await _fetch_ch_safe(TOTALS_CHANNEL)
    out_ch = await _fetch_ch_safe(RECAP_CHANNEL)
    if four_ch is None or totals_ch is None or out_ch is None:
        log.error(f"[AUTORECAP] {kind}: a required channel is unreachable — skipped")
        return False

    recap, has_plays = await build_recap_text(start, end, title, four_ch, totals_ch)
    if not has_plays:
        log.info(f"[AUTORECAP] {kind} for {anchor:%m/%d}: nothing graded — not posting")
        return True   # treat as handled; don't retry all night

    pending = await ungraded_started_plays(start, end)

    # Build the finished embeds here instead of dropping plain text in the
    # recap channel for a mod to copy across — that manual paste was the
    # whole point of automating this.
    data = parse_recap_text(recap)
    if not data:
        log.error(f"[AUTORECAP] {kind}: could not parse own output — "
                  f"falling back to plain text in the recap channel")
        await send_long_message(out_ch, recap)
        return True

    four = data.get("four_plus") or {}
    tots = data.get("totals") or {}
    has_four = four.get("w", 0) + four.get("l", 0) + four.get("wash", 0) > 0
    has_tots = tots.get("w", 0) + tots.get("l", 0) > 0

    if has_four:
        four_only = {
            "title": data["title"],
            "four_plus": data["four_plus"],
            "totals": {"w": 0, "l": 0, "units": 0.0},
        ...
        league_embed = build_league_embed(four_only, color)
        if four_ch:
            await four_ch.send(embed=embed)
            if league_embed:
                await four_ch.send(embed=league_embed)
            log.warning(f"[AUTORECAP] POSTED {kind} 4+ embed -> #{getattr(four_ch,'name','4+')}")
        await mirror_four_recap(four_only)

    if has_tots:
        tots_only = {
            "title": data["title"],
            "four_plus": {"w":0,"l":0,"wash":0,"units":0.0,"nw":0,"nl":0,
                          "cw":0,"cl":0,"kw":0,"kl":0,"rw":0,"rl":0},
            "totals": data["totals"],
            "leagues": [],
        }
        t_embed, _tc = build_recap_embed(tots_only)
        if totals_ch:
            await totals_ch.send(embed=t_embed)
            log.warning(f"[AUTORECAP] POSTED {kind} totals embed -> "
                        f"#{getattr(totals_ch,'name','totals')}")

    # Ungraded reminder stays internal — it's a to-do, not member content.
    if pending and out_ch:
        try:
            await out_ch.send(f"⚠️ **{title}** posted with "
                              f"{len(pending)} finished play(s) still ungraded:\n"
                              + "\n".join(f"• {p}" for p in pending[:12]))
        except Exception:
            log.exception("[AUTORECAP] Could not post ungraded notice:")

    log.warning(f"[AUTORECAP] DONE {kind} — {title}")
    return True


AUTO_RECAP_GRADE_GRACE_HOURS = 2   # a game is "due" a grade this long after tip


async def ungraded_started_plays(start, end, grace_hours=AUTO_RECAP_GRADE_GRACE_HOURS):
    """Ungraded plays in the window whose game time has ALREADY passed.

    The plain ungraded scan flags any line without a ✅/❌/🧼, with no notion
    of when the game is. That breaks for this workflow: the overnight slate
    is posted before midnight and is full of tomorrow's games, all correctly
    ungraded because they haven't happened. Treating those as "pending"
    meant the daily recap could never post on time and always fell through
    to the 2 AM cutoff.

    Only games that have actually finished count as owing a grade.
    """
    ch = await _fetch_ch_safe(FOUR_PLUS_CHANNEL)
    if ch is None:
        return []
    now = datetime.now(EST)
    due_by = now - timedelta(hours=grace_hours)
    pending, seen = [], set()
    try:
        async for msg in ch.history(limit=200):
            mt = msg.created_at.astimezone(EST)
            if start and mt < start:
                break
            if end and mt >= end:
                continue
            raw = _extract_raw_plays(msg.content)   # ungraded play lines only
            if not raw:
                continue
            for line, _tstr, game_dt, _cond in _resolve_slate_dates(raw, mt):
                if game_dt > due_by:
        ...
        log.exception("[AUTORECAP] Pending-grade scan failed:")
    return pending


async def _all_graded_for(start, end):
    """True when every game that has already FINISHED has been graded."""
    pending = await ungraded_started_plays(start, end)
    if pending:
        log.info(f"[AUTORECAP] {len(pending)} finished game(s) still ungraded: "
                 + "; ".join(p[:48] for p in pending[:3])
                 + (" …" if len(pending) > 3 else ""))
    return not pending


def _startup_self_check():
    """Verify the config every background loop depends on actually exists.

    A patch once deleted the auto-recap constants; the loop then raised
    NameError, caught it, and retried forever without ever posting. Checking
    at boot turns that into one loud line instead of a silent stall.
    """
    required = [
        "AUTO_RECAP_HOUR", "AUTO_RECAP_MINUTE", "AUTO_RECAP_CUTOFF_HOUR",
        "AUTO_RECAP_ENABLED", "auto_recap_state", "AUTO_RECAP_STATE_PATH",
        "AUTO_RECAP_CATCHUP_UNTIL_HOUR",
        "REMINDER_CHANNEL", "FOUR_PLUS_CHANNEL", "TOTALS_CHANNEL", "RECAP_CHANNEL",
        "FREE_RECAP_MIRROR_CHANNEL", "TT_CHAT_CHANNEL", "FREE_CHAT_CHANNEL",
        "REVIEW_CHANNEL_A", "REVIEW_CHANNEL_B", "REVIEW_TT_INTERVAL_DAYS",
        "REVIEW_GEN_INTERVAL_DAYS", "DAILY_UNITS_ALERT", "MILESTONE_STEP",
        "STREAK_TIERS", "TIER_UNITS", "engage_state", "reminders",
        "RECAP_BUCKET_BY_GAME_TIME", "REM_NOW_DELETE_MINUTES",
    ]
    g = globals()
    missing = [k for k in required if k not in g]
    if missing:
        log.error("[STARTUP] SELF-CHECK FAILED — missing config: "
                  + ", ".join(missing)
                  + ". Affected features will not run until this is fixed.")
    else:
        log.info(f"[STARTUP] Self-check OK | auto-recap "
                 f"{AUTO_RECAP_HOUR:02d}:{AUTO_RECAP_MINUTE:02d} "
                 f"(cutoff {AUTO_RECAP_CUTOFF_HOUR:02d}:00, "
                 f"enabled={AUTO_RECAP_ENABLED}) | reminders -> {REMINDER_CHANNEL} "
                 f"| recap mirror -> {FREE_RECAP_MIRROR_CHANNEL}")
    return not missing


async def _auto_recap_loop():
    """Fires the end-of-day recap sequence once per day."""
    await client.wait_until_ready()
    await asyncio.sleep(120)
    _auto_recap_load()

    log.info(f"[AUTORECAP] Loop ready — daily at "
             f"{AUTO_RECAP_HOUR:02d}:{AUTO_RECAP_MINUTE:02d} EST, "
             f"grading cutoff {AUTO_RECAP_CUTOFF_HOUR:02d}:00")

    while not client.is_closed():
        try:
            if AUTO_RECAP_ENABLED and not locked:
                now = datetime.now(EST)
                # Anchor = the day being recapped. Before the fire time it's
                # today; after midnight we're finishing yesterday's day.
                if now.hour < AUTO_RECAP_CATCHUP_UNTIL_HOUR:
                    anchor = now - timedelta(days=1)
                else:
                    anchor = now

                fire_at = anchor.replace(hour=AUTO_RECAP_HOUR, minute=AUTO_RECAP_MINUTE,
                                         second=0, microsecond=0)
                cutoff = (anchor + timedelta(days=1)).replace(
                    hour=AUTO_RECAP_CUTOFF_HOUR, minute=0, second=0, microsecond=0)
                stamp = anchor.strftime("%Y-%m-%d")

                if now >= fire_at and auto_recap_state.get("last_daily") != stamp:
                    d_start, d_end, _t = _window_for("daily", anchor)
                    ready = await _all_graded_for(d_start, d_end)
                    if ready or now >= cutoff:
                        if not ready:
                            log.warning(f"[AUTORECAP] Cutoff reached with plays still "
                                        f"ungraded — posting {stamp} anyway")
                        if await _post_auto_recap("daily", anchor):
                            auto_recap_state["last_daily"] = stamp
                            _auto_recap_save()

                            if anchor.weekday() == 6:   # Sunday
                                wstamp = anchor.strftime("%Y-W%W")
                                if auto_recap_state.get("last_weekly") != wstamp:
                                    await asyncio.sleep(3)
                                    if await _post_auto_recap("weekly", anchor):
                                        auto_recap_state["last_weekly"] = wstamp
                                        _auto_recap_save()

                            if _is_last_day_of_month(anchor):
                                mstamp = anchor.strftime("%Y-%m")
                                if auto_recap_state.get("last_monthly") != mstamp:
                                    await asyncio.sleep(3)
                                    if await _post_auto_recap("monthly", anchor):
                                        auto_recap_state["last_monthly"] = mstamp
                                        _auto_recap_save()
                    else:
                        log.info(f"[AUTORECAP] {stamp} ready to post but plays are "
                                 f"still ungraded — waiting (cutoff {cutoff:%I:%M %p})")
        except Exception:
            log.exception("[AUTORECAP] Loop error (continuing):")
        await asyncio.sleep(120)
...
async def _recovery_sweep():
    ...
    while not client.is_closed():
        try:
            if not locked:
                added_before = globals().get("_rem_added_total", 0)
                ...
                tracked = sum(len(b) for b in reminders.values())
                recovered = globals().get("_rem_added_total", 0) - added_before
                if recovered:
                    log.warning(f"[REM] SWEEP recovered {recovered} untracked reminder(s) "
                                f"| {tracked} now tracked")
                else:
                    log.info(f"[REM] SWEEP clean | {tracked} reminder(s) tracked")
        except Exception:
...
def rem_diagnostics(guild_id):
    ...
    lines = ["🔎 **REMINDER ENGINE DIAGNOSTICS** ━━━━━━━━━━━━━━━━━━",
             ...
             f"Pending deletes: `{len(rem_pending_deletes)}` "
             f"(reminders removed after `{REM_NOW_DELETE_MINUTES}m`)",
...
_GRADE_STRIP_RE = re.compile(r'[\u2705\u274c\U0001f9fc\U0001fa9d]')


def _extract_all_plays(text):
    """Like _extract_raw_plays but keeps ALREADY-GRADED lines.

    parse_play_line_for_reminder rejects graded lines by design (no reminder
    for a finished game), but recap bucketing needs to know when a graded
    play's game actually was — so grade marks are stripped for the parse
    probe while the original line is kept for scoring.
    """
    raw = []
    lines = text.split("\n")
    i = 0
    while i < len(lines):
        raw_line = lines[i]
        probe = _GRADE_STRIP_RE.sub("", raw_line)
        play = parse_play_line_for_reminder(probe)
        if play:
            j = i + 1
            ...
            if j < len(lines) and _is_condition_line(lines[j]):
                i = j
            raw.append((raw_line.strip(), play["time_str"], None))
        i += 1
    return raw


_NORM_STRIP_RE = re.compile(r'[\u2705\u274c\U0001f9fc\U0001fa9d\ufe0f\u2622\u26a0\U0001f680]')


def _norm_line(line):
    """Canonical key for matching a play line across passes.

    parse_four_plus rewrites ")❌" to ") ❌" before scoring, while the
    game-time index stores the raw line — so keys built from the two
    disagreed and almost every play was dropped from its own day. Strip
    grade/tier marks and variation selectors entirely so spacing around
    them can never matter again.
    """
    line = _NORM_STRIP_RE.sub("", line)
    return re.sub(r'\s+', ' ', line).strip()


async def game_time_index(channel, start, end, limit=None, pad_hours=36):
    """Lines whose GAME time falls inside [start, end).

    Recaps historically bucketed a play by when its SLATE MESSAGE was
    posted. That breaks whenever a slate is posted on a different calendar
    day than its games — an overnight slate put up at 10:26 PM covers
    tomorrow's card, so those games landed in the wrong day and could fall
    between two daily recaps entirely.

    Scanning a padded message range and filtering on the resolved game time
    puts every play in the day it is actually played.
    """
    keep = set()
    if channel is None:
        return keep
    m_start = start - timedelta(hours=pad_hours) if start else None
    m_end = end + timedelta(hours=pad_hours) if end else None
    try:
        # limit=None means UNLIMITED, matching parse_four_plus. Coercing it
        # to 400 here silently truncated the index for any window older than
        # ~400 messages, so those months lost their entire 4+ record while
        # recent months looked fine.
        async for msg in channel.history(limit=limit):
            mt = msg.created_at.astimezone(EST)
            if m_start and mt < m_start:
                break
            if m_end and mt >= m_end:
                continue
            raw = _extract_all_plays(msg.content)
            if not raw:
                continue
            for line, _tstr, game_dt, _cond in _resolve_slate_dates(raw, mt):
                if (start is None or game_dt >= start) and (end is None or game_dt < end):
                    keep.add((mt.date(), _norm_line(line)))
    except Exception:
        log.exception("[RECAP] Game-time index failed — falling back to message time:")
        return None
    return keep
...
async def parse_four_plus(channel, start, end, limit=None, verify=False,
    ...
    # Bucket by when the game is played, not when the slate was posted.
    if by_game_time is None:
        by_game_time = RECAP_BUCKET_BY_GAME_TIME
    gt_index = None
    if by_game_time and start and end:
        gt_index = await game_time_index(channel, start, end, limit)
        # An empty index would filter out every play and silently report a
        # zeroed month. A genuinely empty window scores zero either way, so
        # falling back to message-time bucketing here can only help.
        if gt_index is not None and not gt_index:
            log.warning(f"[RECAP] Game-time index empty for "
                        f"{start:%m/%d}-{end:%m/%d} — falling back to message time")
            gt_index = None
    pad = timedelta(hours=36) if gt_index is not None else timedelta(0)
    ...
    async for msg in channel.history(limit=limit):
        ...
        if start and not((start - pad) <= msg_time < (end + pad)):
            continue
        ...
        for raw_line in msg.content.split("\n"):
            ...
            # Game-time bucketing: only count lines this window owns.
            if gt_index is not None:
                if (msg_time.date(), _norm_line(line)) not in gt_index:
                    continue
...
async def _resolve_channel(ch_id, label):
    ...
    if ch is None:
        try:
            ...
            log.info(f"[STARTUP] {label} channel {ch_id} no longer exists — skipping.")
            return None
        except Exception as e:
...
async def on_ready():
    ...
    if not _dispatcher_started:
        ...
        _spawn_supervised(_engagement_loop, "engagement")
        _spawn_supervised(_auto_recap_loop, "auto_recap")
            ...
            try:
                ...
                if is_four_ch or is_tots_ch:
                    ...
                    if has_four_plays:
                        ...
                        # Identical copy for free members
                        await mirror_four_recap(four_only)
...
# ==============================
    ...
    if content == "bang" and message.channel.id in BANG_CHANNELS:
        now = datetime.now(EST)
...
@discord.app_commands.choices(sport=[
    ...
    discord.app_commands.Choice(name="\U0001f3c8 CFB", value="general"),
])
@discord.app_commands.choices(period=[
    discord.app_commands.Choice(name="📅 Today", value="today"),
    discord.app_commands.Choice(name="📅 Yesterday", value="yesterday"),
    ...
    async def _test_task():
        ...
        if ch is None:
            try:
                ...
                log.error(f"[TESTREMINDER] Could not fetch channel {rem_ch_id}: "
                          f"{type(e).__name__}: {e}")
                ch = None
        await asyncio.sleep((soon_dt - datetime.now(EST)).total_seconds())
        if ch:
