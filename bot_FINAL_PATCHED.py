...
def make_play_key(league, p1, p2, play["time_str"]):
...
client.run(TOKEN)def parse_line(line):
    # Ignore graded
    if "✅" in line or "❌" in line or "🧼" in line:
        return None
    ...
    try:
        ...
        league = convert_league(line.split("-")[0].strip())

        names_part = line.split("-")[1].split("@")[0].strip()
        if "vs" not in names_part:
            return None

        p1, p2 = [x.strip() for x in names_part.split("vs")]

        import re
        tm = re.search(r'@\s*(\d{1,2}:\d{2})\s*([APMapm]{2})', line)
        if not tm:
            return None

        time_str = f"{tm.group(1)} {tm.group(2).upper()}"
        ...
        if dt < now - timedelta(minutes=90):
            return None

        return (league, p1, p2, dt, time_str)
