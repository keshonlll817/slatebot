...
REMINDER_CHANNEL     = 1442010467345236160   # #tabletennis-chat (main server)
TEST_REMINDER_CHANNEL = 1471792196582637728   # test server STARTING SOON/NOW alerts (test channel)
...
async def _reminder_task(guild_id, key, guild, league, p1, p2, wins, total, tier, game_dt, reminder_channel_id):
    """
...
def _schedule_play_for_message(guild_id, message_id, guild, play, game_dt, reminder_channel_id):
    ...
    task = asyncio.ensure_future(
        _reminder_task(guild_id, key, guild, league, p1, p2, wins, total, tier, game_dt, reminder_channel_id)
    )
...
# ==============================
    ...
    if content.startswith("!reminderremove"):
        ...
        # Build indexed list of active plays (same order as !reminders)
        all_plays_ordered = []
        seen_keys = set()
        for msg_id, msg_tasks in guild_tasks.items():
            for key, task in msg_tasks.items():
                if not task.done() and key not in seen_keys:
                    all_plays_ordered.append((key, msg_id, task))
                    seen_keys.add(key)
        all_plays_ordered.sort(key=lambda x: x[0])

        if not all_plays_ordered:
            await message.channel.send("⏰ No reminders currently scheduled.")
            return

        # Parse indices from command e.g. !reminderremove 1,2,5
        parts = content.replace("!reminderremove", "").strip()
        if not parts:
            await message.channel.send("Usage: `!reminderremove 1,2,5` — use index numbers from `!reminders`")
            return

        try:
            indices = [int(x.strip()) for x in parts.split(",") if x.strip()]
        except ValueError:
            await message.channel.send("❌ Invalid format. Usage: `!reminderremove 1,2,5`")
            return
        ...
        for idx in indices:
            if idx < 1 or idx > len(all_plays_ordered):
                invalid.append(idx)
                continue
            key, msg_id, task = all_plays_ordered[idx - 1]
            task.cancel()
            # Remove from guild task tracking
            if msg_id in scheduled_tasks.get(guild_id, {}):
                scheduled_tasks[guild_id][msg_id].pop(key, None)
            if guild_id in active_keys:
                active_keys[guild_id].discard(key)
            # Parse display name from key
            key_parts = key.split("|")
            if len(key_parts) >= 4:
                removed.append(f"{key_parts[0]} – {key_parts[1]} vs {key_parts[2]} @ {key_parts[3]} EST")
            else:
                removed.append(key)

        response = ""
        if removed:
            response += f"✅ **Removed {len(removed)} reminder(s):**\n"
            for r in removed:
                response += f"• {r}\n"
        if invalid:
            response += f"⚠️ Invalid index(es): {", ".join(str(i) for i in invalid)} — use `!reminders` to see current list"
