    ...
    try:
        ...
        if pre_delay > 0:
            ...
            if ch and (not locked or guild_id == TEST_GUILD_ID):
                text = build_reminder_text(guild, league, p1, p2, wins, total, tier, "STARTING SOON", play_type)
        ...
        if start_delay > 0:
            ...
            # Only send if not locked, or if this is the test server (test reminders always fire)
            if ch and (not locked or guild_id == TEST_GUILD_ID):
                text = build_reminder_text(guild, league, p1, p2, wins, total, tier, "STARTING NOW", play_type)
                await ch.send(text, allowed_mentions=_allowed_mentions_for_guild(guild))
