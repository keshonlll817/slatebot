...
async def send_reminder_confirmation(results, override_channel=None):
    ...
    ch = override_channel if override_channel else client.get_channel(TEST_CHANNEL if BOT_DISABLED else CONFIRMATION_CHANNEL)
    if not ch:
        return
    ...
    if content=="!reminders":
        ...
        for key in sorted(scheduled_tasks.keys()):
            ...
            try:
                ...
                continue
            delta = game_dt_k - datetime.now(EST)
