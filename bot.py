...
async def schedule_message_plays(message, text=None):
    ...
    for line, time_str, game_dt in resolved:
        ...
        # Determine which reminder channel to fire into based on guild
...
async def send_reminder_confirmation(results, override_channel=None):
    ...
    # Route confirmation: test channel posts → test confirmation channel
    #                    main server posts  → main confirmation channel
    if override_channel and override_channel.id == TEST_CHANNEL:
        ch = client.get_channel(TEST_CONFIRMATION_CHANNEL)
    elif override_channel:
        ch = override_channel
    else:
