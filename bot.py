...
locked = False
TEST_GUILD_ID = 1471792196582637728  # Set to your test server's guild ID
...
def build_reminder_text(guild, league, p1, p2, wins, total, tier, label):
    """
    Build the reminder message.
    Uses real role mention if the guild and role are available,
    falls back to plain text otherwise.
    Format: @TT Official LEAGUE – P1 vs P2 EMOJI (wins/total) | LABEL
    """
    if   tier == "nuke":    emoji = " ☢️"
    elif tier == "caution": emoji = " ⚠️"
    else:                   emoji = ""

    body = f"{league} – {p1} vs {p2}{emoji} ({wins}/{total}) | {label}"
...
def parse_play_line_for_reminder(line):
    ...
    Returns dict with keys: league, p1, p2, wins, total, tier, time_str
...
def _resolve_slate_dates(raw_plays, now_est):
    ...
    Rules:
    - Treat the whole block as one slate day.
    - The earliest time in the block is the slate anchor (e.g. 12:00 AM).
    - All times are placed on the same date as the anchor.
    - If a time is numerically earlier than the anchor, it belongs to the
      following calendar day (e.g. a slate that runs 10 PM → 2 AM next day).
    - If the anchor date is more than 23 hours in the past, shift to tomorrow.
    ...
    valid_mins = [m for m in all_mins if m is not None]
    if not valid_mins:
        return []

    slate_start_mins = min(valid_mins)

    # Build the anchor datetime on today's date
    ...
    # If the slate start was more than 23 hours ago, anchor is tomorrow
    ...
    results = []
    for (line, time_str), mins in zip(raw_plays, all_mins):
        if mins is None:
            continue
        try:
        ...
        # If this time is before the slate anchor it belongs to the next day
...
async def _reminder_task(guild_id, key, guild, league, p1, p2, wins, total, tier, game_dt):
    """
    ...
    try:
        ...
        if pre_delay > 0:
            ...
            if ch:
                text = build_reminder_text(guild, league, p1, p2, wins, total, tier, "STARTING SOON")
                await ch.send(text, allowed_mentions=_allowed_mentions_roles())
        ...
        if start_delay > 0:
            ...
            if ch:
                text = build_reminder_text(guild, league, p1, p2, wins, total, tier, "STARTING NOW")
                await ch.send(text, allowed_mentions=_allowed_mentions_roles())
...
def _schedule_play_for_message(guild_id, message_id, guild, play, game_dt):
    """
    ...
    task = asyncio.ensure_future(
        _reminder_task(guild_id, key, guild, league, p1, p2, wins, total, tier, game_dt)
    )
...
async def schedule_message_plays(message, text=None):
    ...
    4. Schedule tasks for each future play.
    5. Return list of result dicts for confirmation message.
    ...
    # Step 4 & 5: schedule and collect results
    results = []
    for line, time_str, game_dt in resolved:
        ...
        key, was_scheduled = _schedule_play_for_message(
            guild_id, message_id, guild, play, game_dt
        )
...
async def on_message(message):
    ...
    # ── Schedule reminders for ANY message in reminder-watched channels ──
    # Runs BEFORE the bot-author check so bot-posted slates are also caught.
...
# ==============================
    ...
    if content=="!reminders":
        ...
        now_est = datetime.now(EST)
        lines   = [f"⏰ **ACTIVE REMINDERS** ({len(all_plays)} play(s)) ━━━━━━━━━━━━━━━━━━"]

        for key in sorted(all_plays.keys()):
            ...
            lines.append(f"{league_k} – {p1_k} vs {p2_k} @ {time_k} EST ({countdown})")
...
# SEND NEW SLATE
    ...
    if four_plus:
        ...
        # Schedule reminders for 4+ plays
        # The sent messages will be caught by on_message above, but we also
        # schedule directly here using the text we built, to guarantee timing.
        # Dedup keys prevent double-scheduling.
        for sent_msg in sent_msgs:
            results_4plus = await schedule_message_plays(sent_msg, text=text)
            if results_4plus:
                conf_ch = message.channel if message.channel.id == TEST_CHANNEL else None
                await send_reminder_confirmation(results_4plus, override_channel=conf_ch)
                break  # confirmation sent once is enough
    ...
    if totals:
        ...
        # Schedule reminders for totals plays
        for sent_msg in sent_msgs:
            results_totals = await schedule_message_plays(sent_msg, text=text)
            if results_totals:
                conf_ch = message.channel if message.channel.id == TEST_CHANNEL else None
